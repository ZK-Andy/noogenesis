...
// 的归纳步在宿主侧（人/会话代理策展），引擎不产基因内容、不接 LLM。候选 = 基因形落状态
// candidates/ 目录，不发事件、不进 select 扫描；入档仍走既有 solidify 闸。
...
import { candidatesDir, eventsDir, capsulesDir, genesDir } from './state.js';

// ---- collect：失败面汇编（只读） -------------------------------------------

// 三类面（序 30 ADR Decision 1/4，范围单源 = 重拍 ADR Decision 2 点名集）：
//   状态 events/*.jsonl 的 outcome=fail 行（入档失败带拒因）；
//   状态 capsules/ 的 outcome.status=fail；
//   状态 genes/ 各基因的 avoid 字段（失败面压缩的既有家）。
...
function collectCapsuleFails(repoRoot: string): { ref: string; evidence: string }[] {
  const root = capsulesDir(repoRoot);
  ...
  for (const ent of fs.readdirSync(root, { withFileTypes: true }).sort((a, b) => a.name < b.name ? -1 : 1)) {
    ...
    for (const f of fs.readdirSync(path.join(root, ent.name)).sort()) {
      ...
      try {
        ...
        throw new EngineError(`${path.relative(repoRoot, root)}/${ent.name}/${f}: not valid JSON`);
      }
...
function candidatePath(repoRoot: string, domain: string, id: string) {
  return path.join(candidatesDir(repoRoot), domain, id + '.json');
}

// 候选落盘：读入即以 gene 校验器验形（封闭 schema，skipDirAnchor——源文件可放
// staging 任意处），再校验 signals 与闸同判据，落状态 candidates/<domain>/<id>.json。
...
function addCandidate(repoRoot: string, candidateFile: string): { report: string; ok: boolean } {
  ...
  // validateGene 对缺席数组宽松（引擎面可选）；候选要与状态基因目录的闸同判据：
  // signals 缺席即拒（否则候选过不了 verify-gene-format，solidify 必红）。
  ...
  if (fs.existsSync(dest)) {
    throw new EngineError(`candidate already exists: ${path.relative(repoRoot, dest)} — delete it explicitly to re-add`);
  }
  assertProtocolIdUnique(candidatesDir(repoRoot), 'candidate', obj.domain, obj.id);
  fs.mkdirSync(path.dirname(dest), { recursive: true });
  fs.writeFileSync(dest, JSON.stringify(obj, null, 2) + '\n', 'utf8');
  return {
    report: `distill: candidate staged at ${path.relative(repoRoot, dest)} (not a gene — archive via solidify after curation)\n`,
    ok: true,
  };
}

function showCandidate(repoRoot: string, domain: string, id: string): string {
  return renderGene(domain + "/" + id, readGene(candidatePath(repoRoot, domain, id)));
}

export { collect, addCandidate, showCandidate, candidatePath };