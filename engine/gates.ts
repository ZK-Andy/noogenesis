// gates.ts — 验证白名单装载（fail-closed）。载体与安全模型：schema ADR S3。
...
function loadGates(engineRoot: string, repoRoot: string) {
  ...
  try {
    ...
    throw new EngineError(`gates.json is not valid JSON — fail-closed: ${(e as Error).message}`);
  }
  const errors: string[] = [];
  ...
  for (const g of Array.isArray(gates) ? gates : []) {
    ...
    if (!Array.isArray(g.args) || !g.args.every((a: any) => typeof a === 'string')) errors.push(`gate ${g.name}: args must be an array of strings`);
    byName.set(g.name, g);
...
function instantiate(gate: any, slots: { outgoing_base: string; head: string }) {
  return {
    name: gate.name,
    cmd: gate.cmd,
    args: gate.args.map((a: string) => a.replace(/\{\{\s*outgoing_base\s*\}\}/g, slots.outgoing_base)
                                  .replace(/\{\{\s*head\s*\}\}/g, slots.head)),
  };
}

export { loadGates, instantiate };
