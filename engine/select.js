'use strict';
// select.js — 信号 → 基因 机械匹配（骨架 ADR D2）：显式喂入、归一化后字面精确匹配、多键取并集。
// Detect 不进引擎：信号发现的判断在人/流程卡侧，引擎只做匹配。

const path = require('path');
const { normalizeSignal } = require('./util');
const { scanGenes } = require('./gene');

// 返回命中 [{ ref, gene }]；signals: 原始键数组（内部归一化）。
function selectGenes(repoRoot, signals, opts = {}) {
...
function runSelect(repoRoot, argv, opts = {}) {
...
module.exports = { selectGenes, formatHits, runSelect };
