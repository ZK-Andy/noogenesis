...
- **行为**：`pre_tool_call` 收到 `write_file` 时，对 `.ts` / `.mts` 且在仓内的目标，用**拟写入内容**跑仓根 oxlint 与 `scripts/verify-export-docs.mts`；命中即回 `{"action":"block","message":…}` 拦回（模型须修完再写）。判据与阈值同 DSH 侧 A4（5s 超时、10 行截断）。
- **降级**：非 `write_file` / 非 `pre_tool_call` / 目标出仓 / 非 `.ts`,`.mts` / 缺 oxlint 或判据件 / 子进程异常或超时 / 坏 JSON —— 一律零输出 exit 0（fail-open，绝不阻断宿主）。判据件故障不是写码方的违规。
...
`patch` 工具（载荷形状未取证）；`pre_llm_call` 上下文注入；Hermes 原生插件面（Python only，撞机器层全栈 TS）；`hermes skills trust` 用户侧一步。
