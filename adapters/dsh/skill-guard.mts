...
export function createSkillGuardPolicies(config: RepoRootConfig, guards: readonly SkillGuardEntry[], deps: SkillGuardDeps = {}): { toolPre: ToolPrePolicy } {
	...
	const toolPre: ToolPrePolicy = (exec) => {
		try {
			...
			const state = store.of<GuardState>(exec.agent?.session, () => ({ seen: new Set(), reminded: new Set(), warned: false }));
			if (exec.name === "skill") {
			...
			const abs = path.resolve(sessionWorkspaceOf(exec) ?? repoRoot, filePath);
			...
			try {
				...
				if (!state.warned) {
					state.warned = true;
					warn(`noogenesis skill guard failed: ${cause instanceof Error ? cause.message : String(cause)}`);
				}
			} catch {
