...
| `maxIndexGenes` | 正整数 / 12；上界 = `MAX_HIT_LINES`（当前 13） | 命中节行数封顶。**装载期 fail-closed 判据**：命中节最坏情形（该行数 × 每行成本〔摘要 160 + 1 截断号 + ref 上界 64 + 分隔 2〕）超 `HITS_SECTION_CHAR_BUDGET`（当前 3044 字符）即拒收——「常驻注入不膨胀」不变量的阻断面（护栏立项 ADR 决定 1；判据、预算、单价与上界单源 = `section.mts`，预算由「已发布缺省 + 一行」推导）。**上界会随单价常量漂移**（摘要宽度 / ref 上界 / 缺省行数）：上调单价即收紧可配行数，显式配置超过新上界的部署升级后被拒载（诊断串带字段名、当前值与上界）——改这三个常量前先核 selftest 的边界断言 |
