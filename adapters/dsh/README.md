...
| `maxIndexGenes` | 正整数 / 12；上界 = 常驻注入预算推导值 13 | 命中节行数封顶。**装载期 fail-closed 判据**：命中节最坏情形（该行数 × 每行成本〔`MAX_SUMMARY_CHARS` + 16〕）超 `HITS_SECTION_CHAR_BUDGET` 即拒收——「常驻注入不膨胀」不变量的阻断面（护栏立项 ADR 决定 1；判据、预算与单价单源 = `section.mts`，阈值 = 已发布缺省的自身最坏情形 + 一行余量 + 溢出行提示）。超预算的旧配置升级后被拒载，诊断串带字段名、当前值与上界 |
