...
- EvoX 有元演化 + 观测，缺轻量可校验的原子 → 我们的 **原子胶囊**（一条规则 / 一张流程卡 / 一个踩坑）就是那个原子。
...
- **演化动作面异步于变更批次**：门禁 / 流程卡 / 规则 / cookbook / 基因的**框架级**改动不随变更批次执行；findings 的类与可机械化候选先归集（家 = [HANDOFF-evolution-pool](../../HANDOFF-evolution-pool.md)），由独立演化轮成批拍板落地（决策 = ADR [2026-09-12-absorption-async-round](../../.agents/notes/implemented/process/2026-09-12-absorption-async-round.md)，动作面 = `feature-flow` §4.6）。
...
2. ~~`validate.yml` 具体复刻哪些 `verify-*`（最小集先跑，还是全覆盖）~~ **已拍板（2026-09-14，批次 5 开题轮 ADR）**：全覆盖为终局——`validate.yml` 实现为穷尽矩阵（gates.json 第一梯队全量 + 白名单外独立件 + `review-tier --enforce` + self-test 抽查 + engine/adapter 双 self-test），与「CI 拥有穷尽矩阵」纪律同向。
3. ~~集体层首批"只读消费公开基因库"起步，还是直接开放贡献 PR~~ **已拍板（2026-09-06）**：只读消费起步（本仓即库、pull → 注入已落地）；贡献 PR 随多人阶段再开（分阶段，ADR 同上）。
4. ~~是否保留"单人队列 → 共享"迁移开关~~ **已拍板（2026-09-06）**：保留——引擎默认离线、`engine pull` 手动入口；adapter `geneBankUrl` 缺省官方库（`false` 显式禁用）、触发点随会话工作区（P2 ADR D4/D7 + bug-fix ADR `2026-09-06-bank-pull-session-trigger`）。
5. ~~与 `dsh-continual-evolve` 的边界：协议层放同一插件还是独立插件~~ **已拍板（2026-09-14，批次 5 开题轮 ADR）**：协议层归属本仓插件包 `noogenesis-dsh` 为终局，旧引擎维持冻结参照（融合立宪 D6）；触发 = 出现第二个真实消费者（多宿主实装 / 外部工具接协议）时重议拆分。融合顺序半边已由记忆线融合立宪（2026-09-10）按机制收口。
