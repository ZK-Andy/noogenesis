...
> 状态：**已收口（2026-09-10）**——本轮冻结为调研档案，收口后不再更新；行动区归还 [HANDOFF-todos.md](../../HANDOFF-todos.md)，终态的 durable 家见 §2.3 收口账。验收口径（「胶囊优化完成」三条件）单源 = [2026-09-10-optimization-round-closure](../../.agents/notes/implemented/process/2026-09-10-optimization-round-closure.md)。
> 开卷说明（2026-09-06）：问题池与相关待办自 [HANDOFF-todos.md](../../HANDOFF-todos.md) 迁入本文件（迁入期间 = 本轮问题与调研的单一事实源）；行动项的完成勾账也在本文件做；durable 结论仍按纪律落 ADR/cookbook/method，本文件不承载决策。
...
## 2. 相关待办与收口账（行动项勾账止于 2026-09-10）

> 迁移说明：以下条目 2026-09-06 从 [HANDOFF-todos.md](../../HANDOFF-todos.md) 行动区迁出，迁出件已删除；本文件是其唯一行动区。**记忆库线两处条目（§2.1 记忆库/记忆系统连接、§2.2-5 开题）2026-09-10 再迁出**：唯一状态家 = [memory-system-dossier.md](../state/memory-system-dossier.md)，本文件不再承载该线。**本文件 2026-09-10 收口冻结**：池内条目全部判终态（落地/判不立/显式挂起），跨会话遗留转行动区——终态与去向见 §2.3。

### 2.1 待办

- [x] **技能集总结**（用户拍板：现在只总结需要哪些技能，后续实施）：已确认需要 = 防过度编码/过度设计（事前，该写）；架构/编码/注释算不算技能随总结再定（不一定是技能）；清单待用户补「等等」项；先例参照 = HERO-Anti-OverDefense（见 §3.7）。〔状态 2026-09-10：防过度已随 M1 批 B 落地（[anti-overdesign](../method/anti-overdesign.md)）；用户拍板暂不新增技能、先补既有技能 references/（§2.2-4/§3.5 合并账，ADR [2026-09-10-skill-references-fill](../../.agents/notes/implemented/process/2026-09-10-skill-references-fill.md)）；「等等」新候选挂起，再收集时重开。〕〔**收口 2026-09-10**：显式挂起——等用户补「等等」候选再重开（「暂不新增技能」的决定在 skill-references-fill ADR）；跨会话遗留 = 行动区（D）条。〕
- [x] **技能使用挂载**（②该不该用/③用了没有）：平台原生挂载点在案——agent/pre-step waterfall 可权威拒绝、tool/call ordered pre 可阻断并回消息、packages/hooks 桥覆盖 prompt 提交/工具前后/停止前；我方适配层仅接 system-prompt 节 + 三工具 + agent/created，上述面全未用。解法方向 = 自建 pre-tool/pre-step 守卫/记录插件；上游同款缺口在案（技能调用仍自觉，repeat-tool-reminder 明示 advice never block）。**求解阶段：先出方案（见 §2.2-1）。**〔状态 2026-09-08：B4 已将首版落为**纯记录件**（A3 观测 `exec.name===\"skill\"`、A6 投影 skill-usage 增量；HERO 判据答案在案）——但 **A8 撤除批随投影面整体退役**（宿主无 ignorable 写入口，fail-closed 拒绝解释会话历史），「用了没有」证据面回归读宿主日志/`tool/result` 事件文本；详 [2026-09-08-b4-mount-wiring](../../.agents/notes/implemented/architecture/2026-09-08-b4-mount-wiring.md) + [A8 撤除](../../.agents/notes/implemented/architecture/2026-09-08-a8-session-record-projection-removal.md)。〕〔**收口 2026-09-10**：③ 已落地 / ② 缓议——③ = session-close 步骤 2 对账步；② 的触发与预拍板在 review-execution-reconciliation ADR；跨会话遗留 = 行动区（D）条。〕
- [x] **评审实质执行在自觉区**（2026-09-06 会话发现、用户拍板记入）：verify-review-brief 机械可判定但无挂载点（gates.py 头注：仅本地预发射，不入 CI、pre-push 不跑——发射评审前无机器事件，靠主会话自觉想起跑）；pre-push/CI 挂载的 review-tier `--enforce` 只强制形式（ADR 带 Review 证据行），评审是否真跑、三路是否真审，机器验不了。推广问题 = git 边界之前的动作序列零挂载点，与「规范事前接入」同根；解法未定。〔状态 2026-09-08：B4 落为纯**记录件**（A4 观测评审机器面运行痕迹、A6 投影 review-surface 累计计数；HERO 判据答案在案）——**A8 撤除批随投影面整体退役**，「评审机器面标记」仍在 `tool/result` 事件文本持久在案，session-close 对账 grep 该面即得；阻断档依 §5 另案过判据。〕〔**状态 2026-09-10 续：拍板落账（讨论轮，按推荐案）**——③ = session-close 步骤 2 扩评审机器面闭集标记对账（「声称 FULL 收口但零标记」即假完成），立即落，ADR [2026-09-10-review-execution-reconciliation](../../.agents/notes/implemented/architecture/2026-09-10-review-execution-reconciliation.md)；② = 收口触点提醒**缓议**（触发 = ③对账暴露真实漏网再立项，接线候选与档位预拍板见 ADR Proposal 2/3）；F3 三路实质 = 语义面不设机器防（确认）。〕〔**收口 2026-09-10**：拍板已落账——③ 落、② 缓议（同上 ADR）；跨会话遗留 = 行动区（D）条。〕
- [x] **规范事前接入**（用户原话：软件架构、编码规范、注释规范等等规范并没有接入协作体系，这也是问题；「我觉得不一定是技能，而是有这个问题，需要事前进行。具体怎么解决，还没有思路」）：问题在案、解决形态未定，待讨论出思路；防上帝类机械校验另条。〔状态 2026-09-08：编码/注释规范已成文单源（[code-standards](../method/code-standards.md) + 根 AGENTS 兜底第 4 条，ADR [2026-09-08-coding-standards](../../.agents/notes/implemented/architecture/2026-09-08-coding-standards.md) implemented）；c2 机器强制已交付（lint 白名单 + export-docs 闸入 gates.json，ADR [2026-09-08-c2-lint-enforcement](../../.agents/notes/implemented/architecture/2026-09-08-c2-lint-enforcement.md)）——「成文 + 机器拦」面闭环；本条余项 = 「事前接入」机制形态（与评审实质执行条同根）；实施计划 = [coding-enforcement-impl-plan.md](coding-enforcement-impl-plan.md) 轨道 A（A4 在环反馈 + 指针）。**2026-09-08 续：轨道 A 已落地**（A4 写码在环 lint 反馈，ADR [2026-09-08-lint-in-loop-feedback](../../.agents/notes/implemented/architecture/2026-09-08-lint-in-loop-feedback.md) implemented + `Review` 行）——「事前接入」机制首版兑现（self-hosting 形态）；余项 = 泛化到按文件所属仓找 lint 配置 + A3 写入前阻断档（升格触发在 ADR）。**2026-09-10 续：架构规范已成文单源**（[architecture-standards](../method/architecture-standards.md)，charter [ADR](../../.agents/notes/implemented/architecture/2026-09-10-architecture-standards-charter.md) implemented，FULL 三审收口）——编码/注释/架构「成文」面齐（A2 开场地图双指针行齐，架构指针行本批接入）；余项不变。〕〔**2026-09-10 续（注释面扩面批）**：在环面从 lint 单判据扩到注释规范——`export-docs-feedback.mts` + 判据件 `verify-export-docs.mts` 文件目标模式，ADR [2026-09-10-export-docs-inloop](../../.agents/notes/implemented/architecture/2026-09-10-export-docs-inloop.md)；注释面「在环缺位」就此补齐（四档矩阵齐）。余项 = lint 泛化 + A3 写入前阻断 + 通用化在环门禁面（三件的触发条件与判不立理由单源 = [lint-in-loop ADR](../../.agents/notes/implemented/architecture/2026-09-08-lint-in-loop-feedback.md) + [export-docs-inloop ADR](../../.agents/notes/implemented/architecture/2026-09-10-export-docs-inloop.md)，本档不复述）。〕〔**2026-09-10 真机复验（0.2.4 装机）**：注释面在环判据全命中——域内（`scripts/`）写缺 JSDoc 导出函数 → 当轮 `block` 替换工具结果（英文违约行）；同文件连续 3 次后第 4 次降级 `context`（写入正常返回、违规行以附注可见）；随后一次合规写入通过并复位计数（再违规复为 block）；域外（`engine/**`）与非 `.ts` 写入零反馈。判据件缺席仓的静默降级 = adapter self-test 夹具覆盖（真机未造此场景）。〕〔**收口 2026-09-10**：已落地——成文三篇 + 机器拦 + 在环两判据；余项三件（lint 泛化 / A3 写入前阻断 / 通用化在环面）= 触发未满足或判不立，单源 = lint-in-loop ADR + export-docs-inloop ADR。〕
- [x] **上帝类预防闸**：单文件行数/依赖扇出上限类机械门禁（engine 现状健康——最大 selftest.js 486 行、核心命令 ≤170，属预防非治病）；静态可计算 → 挂载即零自觉真强制（§3.4 谱系）；对应设计稿 §10「机械校验（scope/blast-radius）」面。〔状态 2026-09-08：B4 C15 评估**判不立**——实测 n=38 行数 p50=152/p90=516/max=1158 无自然拐点、零失控件，立闸 = 防 speculative（HERO-O）；触发条件 = 真实失控件出现（改写困难/评审反复抓同一文件）时以 max×1.5 为候选阈值再过判据，见 [B4 ADR Decision 6](../../.agents/notes/implemented/architecture/2026-09-08-b4-mount-wiring.md)。〕〔**收口 2026-09-10**：判不立——触发阈值单源 = B4 ADR Decision 6。〕
- 记忆库/记忆系统连接 → 已迁出（2026-09-10）：唯一状态家 = [memory-system-dossier.md](../state/memory-system-dossier.md)。
- [x] **事实面定案（免行动留档）**：①懒加载已实证——技能目录只发 ≤500 字摘要、正文经 skill 工具按名注入（设计稿 §8 as-needed 生效），rank 600 = 最弱 bundled 层、同名静默遮蔽，兜底位设计安全非缺陷；②技能脱离本仓可运行性分级实证——2 纯方法论 / 3 半绑定 / 2 实质绑定，「正文可带走、机器检查带不走」，SKILL.md 宿主口径行已自认，免行动。〔**收口 2026-09-10**：免行动留档——实证语料随本档冻结（§3）。〕

### 2.2 方案设计工作面（2026-09-06 起进入求解阶段——材料已集齐，先出方案，无拍板项）

> 阶段说明：语料与材料收集完毕（§3 全部调研 + HERO 细读），现在开始设计解法；以下是方案设计的切入面，不是决策点——每项先出方案再讨论取舍。
>
> **全七面收口（2026-09-10）**：1/2 已落地并真机复验；3 逐项落地或判不立；4 拍板暂不新增技能；5 迁出档案页；6 随框架重建收官；7 完成使命。终态去向见 §2.3。

1. **技能使用守卫的方案设计**：基于 §3.4 挂载面证据（pre-tool/pre-step/钩子桥）设计「守卫/记录插件」的方案——它管什么、怎么挂、拦了之后回什么消息、记录落哪里。②「该不该用」③「用了没有」的解法从这里出。〔状态 2026-09-10 设计轮：**方案已拍板 = 三件套 v0**——①技能路标（A2 开场地图加任务型→技能名映射行）②触点提醒（tool-pre 按路径模式判「该载未载」→ `agent.inject` 一行 advice，**不阻断**，repeat-tool-reminder 同款哲学）③事后对账（session-close 流程卡 grep `tool/result` 技能痕迹，裁断留人/评审）；**阻断档判不立**（「该不该用」是语义判断机器判不了，HERO 判据）。实施待开（实现批带 ADR）。〔**状态 2026-09-10 续：批 A 守卫三件套已收口**——实现 `206ad18` + 评审收口 `51234bc`（FULL 三审 R1 0B/3S、R2 0B/8S、R3 3B/5S 全采纳）+ ADR [2026-09-10-m1-guard-anti-overdesign](../../.agents/notes/implemented/architecture/2026-09-10-m1-guard-anti-overdesign.md) implemented（分批翻转批注）；真机复验随下一次插件发版/重装。批 B（防过度）实现待开。〕〔**状态 2026-09-10 真机复验（0.2.4 装机）**：三件全命中——A2 地图技能路标行在场（会话子树地图含「任务型→技能名」行）；A3 触点提醒对 `docs/` 写码投递一行 advice（不阻断、每会话每技能至多一次、载入对口技能后不再提醒）；session-close 对账步可跑。〕
2. **防过度编码/过度设计的方案设计**：以 HERO 细读（§3.7）为参照设计解决形态——技能 / 常驻注入块（HERO 是常驻档实例，作者自认非 enforcement）/ 门禁补挂 / 混合，先用 §3.4 谱系逐档过一遍再定。〔状态 2026-09-10 设计轮：**形态已拍板 = 混合 D（HERO 原生分层）**——精简契约块（~8 条 + 核心判据一句 + Rule 8 反 stalling 补空白）进根 AGENTS（预算内）+ 完整蒸馏（9 规则 + 6 校准形状 + prose-standard/trim-cot-leakage 同域归并）成 docs/method 新篇 + **cases 不加载留辩论**（评审按 ID 质询「与该案例有何不同」）；门禁补挂判不立（HERO-O 本尊，C15 同型判据）；预期定「护栏非开关」。实施待开（实现批带 ADR）。〔**状态 2026-09-10 续：批 B 防过度已收口**——实现 `5b0ad9b` + 评审收口 `d277c47`（FULL 三审 R1 1B/3S、R2 3B/6S、R3 5B/4S 全采纳）；[anti-overdesign](../method/anti-overdesign.md) + 根 AGENTS 契约块 9 条（547/800）+ [cases 索引](anti-overdefense-cases-index.md) 18 例 + 评审检查项第 5 条；ADR 第二条 Review 行在案。M1 两批闭环。〕〔**状态 2026-09-10 真机复验（0.2.4 装机）**：防过度契约块常驻在场（根 AGENTS「防过度设计（范围契约）」节随基座注入会话）；评审检查项第 5 条口径在清单；cases 索引按设计不进加载面。〕
3. **小落差评估**：钩子自动安装 / 门禁 DAG 并行 / CI 文件名契约闸 / archived-notes 校验件 / 技能 references/ 形态——逐项评估要不要纳入及何时纳入。
4. **技能清单补全**：第一题的「等等」项——架构/编码/注释算不算技能、还缺什么，与用户继续收集归纳（先例参照 = HERO）。〔状态 2026-09-10：架构/编码/注释均已按规范篇落地（非技能形态）——[architecture-standards](../method/architecture-standards.md) + [code-standards](../method/code-standards.md)；同日用户拍板**暂不新增技能**（用不上不写），先给既有技能补 references/——§2.2-3 的实拆触发已发生（携带示例库/扫描清单），noo-trim-cot-leakage + noo-prose-standard 补 `references/`（示例库 + 扫描电池），noo-doc-standards 维持不拆（内容家在 [docs/method](../method/doc-standards.md)），ADR [2026-09-10-skill-references-fill](../../.agents/notes/implemented/process/2026-09-10-skill-references-fill.md)；「等等」新技能候选清单挂起，再收集时重开。〕
5. **记忆库线开题** → 已迁出（2026-09-10）：唯一状态家 = [memory-system-dossier.md](../state/memory-system-dossier.md)。
...
| §2.2-5 记忆库线 | 已落地（迁出转档） | [memory-system-dossier](../state/memory-system-dossier.md) |
...
〔**状态 2026-09-08（B0/B1/B3/B5 逐项落地）**：钩子自动安装 → **lefthook 内建 postinstall 落地**（B3，devDependency，`lefthook install -f`，非自研安装器）；门禁 DAG 并行 → **gates.mts DAG runner 落地**（B1：needs/after + 有界并行 + fail-closed 图校验）；archived-notes 校验件 → **verify-archived-agent-notes.mts 落地**（B0：封闭类树 + freeze 清单 append-only）；技能 references/ 形态 → **判据落地不实拆**（B0：目录非空 + SKILL.md 相对链接；noo-doc-standards 实测 377 词无拆分需求，实拆随真实需求另起）；〔**状态 2026-09-10 续：实拆触发兑现**——trim-cot-leakage/prose-standard 携带示例库/扫描清单，两件补 `references/`（ADR [2026-09-10-skill-references-fill](../../.agents/notes/implemented/process/2026-09-10-skill-references-fill.md)）；doc-standards 维持不拆〕；CI 文件名契约闸 → **C15 判不立**（B4 Decision 6：全历史 `--diff-filter=A` 零构建产物入库 + `.gitignore`/CI/评审三层已盖 = 防 speculative，HERO-O；触发条件在案）。〕
