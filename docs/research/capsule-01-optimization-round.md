...
2. **防过度编码/过度设计的方案设计**：以 HERO 细读（§3.7）为参照设计解决形态——技能 / 常驻注入块（HERO 是常驻档实例，作者自认非 enforcement）/ 门禁补挂 / 混合，先用 §3.4 谱系逐档过一遍再定。〔状态 2026-09-10 设计轮：**形态已拍板 = 混合 D（HERO 原生分层）**——精简契约块（~8 条 + 核心判据一句 + Rule 8 反 stalling 补空白）进根 AGENTS（预算内）+ 完整蒸馏（9 规则 + 6 校准形状 + prose-standard/trim-cot-leakage 同域归并）成 docs/method 新篇 + **cases 不加载留辩论**（评审按 ID 质询「与该案例有何不同」）；门禁补挂判不立（HERO-O 本尊，C15 同型判据）；预期定「护栏非开关」。实施待开（实现批带 ADR）。〕
