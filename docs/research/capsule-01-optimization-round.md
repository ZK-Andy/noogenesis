...
6. **门禁运行时语言评估**（2026-09-06 用户提出并定调方向；同日用户纠正「语义门禁」误判）：
   - 用户原话：「如果ts能完全解决问题，我反而不太喜欢用python。执行速度上感觉ts更快一点」「这一次我们要在优化胶囊的时机，彻底解决问题。我们npm发包，都有node，直接跑nodejs 看来是最好的路径」「我觉得不对，为什么我们不需要语义门禁呢？我们没有是因为我们不会写啊，为什么是不需要呢？」。
   - **agent 错误结论（已撤回）**：「我方门禁检查对象 = JSON/Markdown/正则/git，无一处需要 TS 语义」——**没有 ≠ 不需要**。我们没有语义门禁是因为从未建设该能力。我方吃过语义亏的实锤：①`register(t1,t2,t3)` 单参合同 bug（调用签名形状错、静默丢参——对宿主 .d.ts 的类型核对闸可机械抓住）；②evaluate 意外并入缓存宇宙（作用域可见性事实）；③gene_sha 双实现镜像（JS/Python 双协议实现靠 self-test 兜底，自认脆弱面）。
   - **上游选 tsx 的原因（源码实证，2026-07-14 ADR）**：语义门禁用 `ts.Program`+`TypeChecker` 提取强类型事实 + 门禁与产品共享模块 + 工具链锚定（postinstall 使 node_modules 弱点不存在）。检查 TS 代码时最权威的解析器就是 TS 编译器。
   - **修正后的求解框架**（两个独立子题）：①**语义门禁能力是否建设**——候选场景 = 适配层对宿主 API 签名核对 / 引擎作用域与契约一致性 / 双实现镜像机械比对 / gates.json 命令引用存在性；②**语言决策随①而定**——本地快检 = 零依赖纯 Node JS（无转译、裸 checkout 照跑、与 engine 同栈，此结论不变）；CI 穷尽矩阵侧可引入 tsc 语义门禁（devDependency 在 CI 在场，不进钩子快检路径）。**Python 退役的方向不变**；「TS 完全无用」的说法撤回——tsx 每门禁一份转译税的形态仍然不对，语义能力以 CI 侧 tsc 门禁形态按需建设。
   - **实施面（待方案细化）**：若①拍板建设，语义门禁走 CI 矩阵 + `scripts/ts-project.ts` 同款模式（上游有完整先例可蒸馏）；快检族迁移 = 10+ verify 脚本 + change-scope + gates.py 发射器 + self-test 移植，FULL 档批次逐脚本兜底。
