...
- [ ] **技能集总结**（用户拍板：现在只总结需要哪些技能，后续实施）：已确认需要 = 防过度编码/过度设计（事前，该写）；架构/编码/注释算不算技能随总结再定（不一定是技能）；清单待用户补「等等」项；先例参照 = HERO-Anti-OverDefense（见 §3.7）。〔状态 2026-09-10：防过度已随 M1 批 B 落地（[anti-overdesign](../method/anti-overdesign.md)）；用户拍板暂不新增技能、先补既有技能 references/（§2.2-4/§3.5 合并账，ADR [2026-09-10-skill-references-fill](../../.agents/notes/implemented/process/2026-09-10-skill-references-fill.md)）；「等等」新候选挂起，再收集时重开。〕
