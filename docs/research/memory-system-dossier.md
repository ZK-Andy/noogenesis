...
- [ ] **融合轮待拍板**（开题与接口契约已拍 2026-09-10）：① 机制代码归属（接口面收敛后倾向本仓，待确认）② 痕迹提炼面归属 ③ 实现顺序与第一期交付 + 行为评估自建形态与第二期范围——契约与吸收补丁清单见 [融合立宪 ADR](../../.agents/notes/proposed/architecture/2026-09-10-memory-line-fusion-charter.md)。痕迹提炼 v0 候选（session-close 摩擦点寻址，零引擎改动）若提前独立先跑，须落在该 ADR D4 的触发与对象口径内。
