...
- [ ] **融合轮第一期实现批待启动**（范围已拍 2026-09-10，D10）：IN = D8 观测输入面 + 派生与 Select 建议档 + 凭据筛查；OUT = 行为评估自建（第二期）/ 无对象存储与并发守卫（判不立）/ 宿主挂载面（暂不立）/ 痕迹提炼面（挂起）。启动时另立实现 ADR（写死两个设计点：输入面写入触发点 · Select 输出契约改法）。契约与判据见 [融合立宪 ADR](../../.agents/notes/proposed/architecture/2026-09-10-memory-line-fusion-charter.md)。
