...
- [x] **融合轮第一期实现批已落地**（2026-09-11，D10）：观测输入面（`observe` 命令 → `.noogenesis/observations/`）+ 派生与 Select 建议档（`advice:` 行）+ 凭据筛查（`scripts/verify-secrets.mts`，三面）——[实现 ADR](../../.agents/notes/implemented/architecture/2026-09-11-memory-line-phase1-observation-face.md)（两个设计点写死：写入触发点 = 人 / 显式目标 · Select 输出契约 = 追加式建议档）。OUT 面不变：行为评估自建（第二期）/ 无对象存储与并发守卫（判不立）/ 宿主挂载面（暂不立）/ 痕迹提炼面（挂起）。契约与边界见 [融合立宪 ADR](../../.agents/notes/proposed/architecture/2026-09-10-memory-line-fusion-charter.md)。
