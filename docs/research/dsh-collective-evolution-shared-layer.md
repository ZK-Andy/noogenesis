...
```
 ...
 ├────────────────────────────────────────────────────────┤
 │ ② 协议层   ← 新增（核心缺口）                           │
 │    把资产规范成 gene/capsule：schema+出处+版本+验证签名   │
...
### 6.2 gene/capsule Schema（协议化的最小可交换对象）
...
1. **P0（本设计落地）**：定 gene/capsule **Schema** + 写 `validate.yml` 验证清单（把 `verify-*` 移植过来），把既有资产（AGENTS/skills/ADR/cookbook）**协议化成首批基因**。
2. **P1**：引擎插件实现 ②协议层 + ④注入层，**完全本地/离线可用**；验证闸接入本地基准。
3. **P2**：共享客户端（⑤）——git fetch/pull 消费 + 本地验证过才 PR；跑通"贡献→CI→合入→pull 复用"闭环。（2026-09-06 拍板回写，ADR `2026-09-06-p2-shared-consumer`：首批 = 本仓即库 + 只读消费起步，`engine pull` + `manifest.json` 索引 + 缓存合并扫描已落地；贡献 PR 与观测面随多人阶段。）
...
> 2026-09-06 回写：1/3/4 已由 ADR `2026-09-06-p2-shared-consumer` 拍板收口（结论见各条）。2026-09-14 回写：2/5 已由批次 5 开题轮 ADR（`2026-09-14-batch5-opening-round`）拍板收口（结论见各条）。
...
2. ~~`validate.yml` 具体复刻哪些 `verify-*`（最小集先跑通，还是全覆盖）~~ **已拍板（2026-09-14）**：全覆盖为终局——`validate.yml` 实现为穷尽矩阵，与「CI 拥有穷尽矩阵」纪律同向（结论与证据见批次 5 开题轮 ADR）。
...
5. ~~与 `dsh-continual-evolve` 的边界：协议层放同一插件还是独立插件~~ **已拍板（2026-09-14）**：协议层归属本仓插件包 `noogenesis-dsh` 为终局（触发 = 出现第二真实消费者时重议拆分；结论见批次 5 开题轮 ADR）。
