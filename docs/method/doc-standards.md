...
7. **流程卡与阶段卡写成步序，不写成名词清单**：每个阶段必须写明**步序**（谁先谁后）、**角色位**（哪步是人、哪步是 agent）、**关口**（哪一步停下等确认/等闸过，未过不得继续）。只有出口清单与产出物的阶段会被按最低成本解释——「产出 = 某物」不构成完成判据。（出处 = ADR [2026-09-11-stage-card-structure](../../.agents/notes/implemented/process/2026-09-11-stage-card-structure.md)；评审兜底 = 根 AGENTS「评审检查项」第 6 条。）
