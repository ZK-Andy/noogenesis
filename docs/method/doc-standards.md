...
- **骨架四段（封闭集，按序）**：`## 决策指针`（只列指针，每条带相对链接）→ `## 行动区`（`- [ ]` / `- [x]` 条目，open ≤12）→ `## 触发条件`（待观察面：未到触发的项）→ `## 状态日志`（已确认面：`- YYYY-MM-DD｜` 条目，≤40 条，超限把最旧条目压缩为一行）。第五个 `##` 节违规——新素材归其 tier 的家。痕迹形态只在四段内表达，不另立状态标签。
...
- **上限依据**【探索性】：行动区 open 12 = 比全局行动区的 16 更紧一档（主题级应小于全局）；状态日志 40 ≈ 试点件实增长速率（8 天 10 条）下约一个月量级——超限即压缩最旧条目。
