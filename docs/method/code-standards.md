...
> 档位速查：每条规则首列标注 `[M]` 机器拦 / `[W]` 工具警告 / `[I]` IDE 提示 / `[R]` 留评审（见 [standard-authoring](standard-authoring.md) §1）；**能升档就升档**（c2 oxlint 落地后逐条升）。

## 1. 强制力度分档

按 [standard-authoring](standard-authoring.md) §1：`[M]` 机器拦（verify-*/CI，违约 fail loud）、`[W]` 工具警告（oxlint/tsc，保持零警告）、`[I]` IDE 提示、`[R]` 留评审（机器无法强制 → 列 [review.md](review.md) §5 AI 兜底清单）。每条规则声明档位与「为什么停在这档」。

## 2. 注释规范（主要内容）

> 判据家：何时需要注释 = [noo-prose-standard](../../.agents/skills/noo-prose-standard/SKILL.md)「按位置定必需覆盖」；本节约束**写法**与**不写什么**。散文铁律（每事实一个家/写当前状态/禁变更史）见 [doc-standards](doc-standards.md) §2，注释是代码里的散文，同样适用。

### 2.1 公共 API 契约注释 `[R]`（c2 升 `[W]`/`[M]`）
...
- 行注释 `//` 单行；块注释 `/** */` 仅用于文件头注与公共契约注释。
- 头注 = 每文件首块 `/** */`：一句话职责 + 关键契约/纪律指针 + 归属 ADR（如有）；不长篇叙事。
...
- **闭集常量**：魔法数字/字符串必须命名常量；封闭集用 `as const` 数组 + 派生联合类型（见 `adapters/dsh/mount-policies.mts` 的 `SUBTREE_AGENTS` 实例）。
...
本节「留评审」档条目（2.1–2.3、3）是 [review.md](review.md) §5 AI 兜底清单的一部分；评审代理按根 [AGENTS.md](../../AGENTS.md)「评审检查项」核对本节判别式在 diff 面是否被违反。c2 工具落地后逐条升档移出。

## 6. 升档路径（c2）

- oxlint 接入后：2.4 → `[W]`；2.1 公共契约注释 → `no-warning-comments`/JSDoc 档位升 `[W]`/`[M]`；3 部分可判定的（未用变量、`as const` 可判定面）随 lint 规则升。
