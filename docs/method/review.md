...
**发射前机械闸**：每路简报写毕、评审代理发射前，主会话实跑 `node scripts/verify-review-brief.mts --enforce`（结构合规 + 门禁自证耦合；所需 lane 集由简报自身 base..head 的档位推导——FULL→R1/R2/R3，LIGHT→R2，`--lanes` 可显式覆盖）；`--lanes` 显式给出时**只判这些泳道**——目录里已收口泳道的残留简报（未归档、或 head 已滞后于 HEAD）不拦余下泳道的发射。简报缺项、自证含非 0 exit、简报声明的 `base` 不可解析、或 `head` 未钉住本仓 HEAD（悬空/异对象 ref 与 HEAD 前移都会让评审对象与记录漂移）即发射被拦——两条 ref 判据都是**发射前判据**（评审期间不得提交，故发射时 head 恒等于 HEAD）。
