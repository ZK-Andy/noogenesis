...
- **注释面在环扩面已落地**（ADR [2026-09-10-export-docs-inloop](.agents/notes/implemented/architecture/2026-09-10-export-docs-inloop.md) implemented，FULL 三审全采纳）：A4 在环面 = lint + 注释面两判据（`export-docs-feedback.mts` 跑仓内判据件 `scripts/verify-export-docs.mts <file>` 文件目标模式，域归属单源；死锁门 `createBlockGate` 折叠单源）；模型面违约行英文（口径单源 = `adapters/dsh/README.md`）。真机复验随下一发版。
