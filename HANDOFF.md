...
- 2026-09-13｜**护栏建设轮余项收尾批**：决定 1 建议行落地 = `token-baseline.mts` 自会话第二步一行宿主读数（`ctx.get` 懒取用 / 缺席静默 / 失败一条 warn）+ selftest 7 条；决定 2 两本账落点落账，发版后修复账三次实发已回填；勘误两处（类型断言跨代依赖 ERESOLVE → 运行期形状闸；直读服务缺席即抛、首步漏系统提示面）。README 已同步。
