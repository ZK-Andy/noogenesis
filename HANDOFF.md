...
> **⏭ 下一步：按「设计稿功能补全批次表」逐批开工**——第 1 批 = 演化原语（Capsule / Mutation / Event 扩字段 / blast-radius / 候选比较）；46 项有序、批次 3/4 须先立重拍 ADR，单源 = [批次表 ADR](.agents/notes/proposed/architecture/2026-09-13-feature-completion-backlog.md)（proposed）。其余候选（演化轮 / 发版 0.2.7 / `capsules/` 路径两义 / 技能清单候选）触发与判据见 [HANDOFF-todos](HANDOFF-todos.md) D 条与 [池件](HANDOFF-evolution-pool.md)。最新批 = 2026-09-13 **档案页制度批（FULL 三审）**：立 `docs/state/` tier + 骨架四段机器闸（含重复段判据）+ 摩擦点落点表单源；推广 = 判据立、页不立，[制度 ADR](.agents/notes/implemented/process/2026-09-13-dossier-institution.md)。前序批次见下方滚动窗。
