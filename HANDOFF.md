...
> **⏭ 下一步：批次 1 序 3 = Event 扩字段（`mutation_id`/`capsule_id`/`env_fingerprint`/`validation_report_id`）**——批次 1（演化原语）序 1 Capsule、序 2 Mutation 均已收口，余序 3 Event 扩字段 / 序 4 blast-radius / 序 5 候选比较；46 项有序、批次 3/4 须先立重拍 ADR，单源 = [批次表 ADR](.agents/notes/proposed/architecture/2026-09-13-feature-completion-backlog.md)（proposed）。其余候选（演化轮 / 技能清单候选 / Detect 逐源解除）触发与判据见 [HANDOFF-todos](HANDOFF-todos.md) D 条与 [池件](HANDOFF-evolution-pool.md)。最新批 = 2026-09-13 **批次 1 序 2 Mutation 原语（FULL 三审 R1 0B/2S、R2 0B/4S、R3 0B/4S；全采纳）**：`mutations/<domain>/<id>.json` 封闭六字段 + `mutation add|show` + 闸件扩展与复算，重拍 schema S2（kind 五件、键集三面），[Mutation ADR](.agents/notes/implemented/architecture/2026-09-13-mutation-primitive.md)。前序批次见下方滚动窗。
