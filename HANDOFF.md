...
- 2026-09-08｜**B4 实机重验 + M1/M2 修复收口（FULL 三审全采纳；ADR `2026-09-08-mount-exec-arguments-field` implemented；0.2.1 发版）**：持久化容忍度/A2/A6→A8/M3 真机全绿；根因 = exec 字段 `arguments` 被误读 `exec.args`（夹具复刻假设 = 冒烟假绿教训）；B4 ADR 三处同步/勘误。**节点：恢复确认随 0.2.1 重装批（todos B 条）。** README 无变更。
