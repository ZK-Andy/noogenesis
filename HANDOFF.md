...
- 2026-09-06｜**桌面部署缺陷修复 + 0.1.3 发版（FULL 三审全采纳；ADR `2026-09-06-bank-pull-session-trigger` implemented）**：0.1.2 重验暴露装载期 pull 错位落地（技能面静默失效）→ 触发点随会话工作区 + `geneBankUrl` 缺省官方库/`false` 禁用；npm latest + tag `v0.1.3`（tag 豁免留痕）；CI 绿。**节点：desktop 重装重验待用户，通过即开胶囊 01 优化轮。**
