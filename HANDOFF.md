...
> **⏭ 下一步：批次 1 序 2 = Mutation 原语**——批次 1（演化原语）序 1 Capsule 已收口，余序 2 Mutation / 序 3 Event 扩字段 / 序 4 blast-radius / 序 5 候选比较；46 项有序、批次 3/4 须先立重拍 ADR，单源 = [批次表 ADR](.agents/notes/proposed/architecture/2026-09-13-feature-completion-backlog.md)（proposed）。其余候选（演化轮 / 发版 0.2.7 / `capsules/` 路径两义 / 技能清单候选）触发与判据见 [HANDOFF-todos](HANDOFF-todos.md) D 条与 [池件](HANDOFF-evolution-pool.md)。最新批 = 2026-09-13 **批次 1 序 1 Capsule 原语（FULL 三审 R1 0B/4S、R2 2B/2S、R3 0B/3S；全采纳）**：`capsules/<domain>/<id>.json` 封闭七字段 + `capsule add|show` + 闸件扩展与复算，重拍骨架 D3 产出面与 schema S2 kind 集，[Capsule ADR](.agents/notes/implemented/architecture/2026-09-13-capsule-primitive.md)。前序批次见下方滚动窗。

## 交接更新记录（摘要滚动窗）

> 滚动窗有界（≤24 条、每条 ≤260 字，机器强制）：只保近期会话批次的**摘要**（日期｜类型｜ADR 指针｜一句话结论），全文下沉 journal；durable 结论在 ADR/cookbook/README/AGENTS，此处不复述。

- 2026-09-13｜**批次 1 序 1 Capsule 原语（FULL 三审 R1 0B/4S、R2 2B/2S、R3 0B/3S；全采纳）**：`capsules/` 封闭七字段 + `capsule add|show` + 闸件 capsules/ 一节与复算；重拍骨架 D3 与 schema S2 两项。[ADR](.agents/notes/implemented/architecture/2026-09-13-capsule-primitive.md)。README 有变更（六→七命令）。
