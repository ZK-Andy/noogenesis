...
> **⏭ 下一步：M1 批 A（守卫三件套）已收口（FULL 三审全采纳，ADR 翻转 implemented）→ 批 B（防过度混合 D）实现待开**。其余候选：优化轮问题池余项（技能清单补全 / 技能挂载守卫真机复验）[capsule-01-optimization-round.md](docs/research/capsule-01-optimization-round.md) / 记忆库线（方向已拍板 = 融合轮 2026-09-10，待融合轮排期；唯一状态家）[memory-system-dossier.md](docs/research/memory-system-dossier.md) / release 工具族首验（B 条，随下一发版）/ 护栏延后收口（D 条，触发 = 胶囊优化后）；随手待办 [HANDOFF-todos.md](HANDOFF-todos.md)。
...
- **M1 守卫三件套已落地（批 A）**（ADR [2026-09-10-m1-guard-anti-overdesign](.agents/notes/implemented/architecture/2026-09-10-m1-guard-anti-overdesign.md) implemented，FULL 三审全采纳）：A2 开场地图技能路标行（存在性过滤）+ A3 触点提醒 advice 档（skill-guard 件 + `config.skillGuards` 映射表，每会话每技能至多一次）+ session-close 技能痕迹对账步；批 B（防过度混合 D：根 AGENTS 契约块 + method 新篇 + cases 索引）实现待开。
