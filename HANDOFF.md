...
- 2026-09-06｜**git 前置条件写明 + 引擎诊断分流（LIGHT 档 R2 单路；ADR `2026-09-06-git-prerequisite-and-diagnosis` implemented）**：前置条件入两 README；gitRoot ENOENT 分流（git 未装不再误报非 git 仓）；R2 抓出夹具裸名 spawn 误伤 + selftest.js 直跑 no-op 假绿（入 cookbook）。落 main，随下次发版出包（用户拍板不发版）。

- 2026-09-06｜**桌面部署缺陷修复 + 0.1.3 发版（FULL 三审全采纳；ADR `2026-09-06-bank-pull-session-trigger` implemented）**：0.1.2 重验暴露装载期 pull 错位落地（技能面静默失效）→ 触发点随会话工作区 + `geneBankUrl` 缺省官方库/`false` 禁用；npm latest + tag `v0.1.3`。**节点：重装重验通过；下一轮 = 胶囊 01 优化轮。**
