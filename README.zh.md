...
- **DSH 适配层**——本仓即 DSH 插件包 [`noogenesis-dsh`](https://www.npmjs.com/package/noogenesis-dsh)@0.2.5（AGPL-3.0）：经 spawn CLI 单合同把引擎接进会话生命周期；7 个 `noo-*` 技能随基因库缓存分发，但宿主当前未注册该 provider，自举仓之外技能不进会话目录（[ADR](.agents/notes/proposed/bug-fix/2026-09-12-bank-skill-provider-registration.md)）。
