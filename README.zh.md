...
- **DSH 适配层**——本仓即 DSH 插件包 [`noogenesis-dsh`](https://www.npmjs.com/package/noogenesis-dsh)@0.2.9（AGPL-3.0）：经 spawn CLI 单合同把引擎接进会话生命周期；7 个 `noo-*` 技能随基因库缓存分发，provider 经 `ctx.inject(["skills"], …)` 可重试路径注册（skills 服务晚到同样落位），提醒面只点名本会话目录里实际可载的技能（[ADR](.agents/notes/implemented/bug-fix/2026-09-12-bank-skill-provider-registration.md)）。
