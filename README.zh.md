...
- **DSH 适配层**——本仓即 DSH 插件包 [`noogenesis-dsh`](https://www.npmjs.com/package/noogenesis-dsh)@0.2.8（AGPL-3.0）：经 spawn CLI 单合同把引擎接进会话生命周期；7 个 `noo-*` 技能随基因库缓存分发，provider 经 `ctx.inject(["skills"], …)` 可重试路径注册（skills 服务晚到同样落位），提醒面只点名本会话目录里实际可载的技能（[ADR](.agents/notes/implemented/bug-fix/2026-09-12-bank-skill-provider-registration.md)）。
...
├── adapters/dsh/           # DSH 适配层（插件壳接线：system-prompt 节 / noo_* 工具 / solidify 人工确认 / geneBankUrl 惰性 pull / 技能随库分发 / 插件日志落盘（`<DSH_HOME>/logs/noogenesis.log`） / 挂载面 A2–A6 五点接线与 A2 开场地图（A6 停止前续跑能力位见 [A6 ADR](.agents/notes/implemented/architecture/2026-09-13-a6-turn-stopping-mount.md)；A8 记录投影已撤，见 [撤除 ADR](.agents/notes/implemented/architecture/2026-09-08-a8-session-record-projection-removal.md)）；B2 起源码 TS，npm 包 = tsc dist）
├── .noogenesis/           # 演化状态面——tracked：genes/ events/ capsules/ mutations/ candidates/ manifest.json；ignored：genes-cache/ observations/
...
可执行门禁清单**单源**于 `engine/gates.json`：`node scripts/gates.mts --list` 发射、`--run` 运行，hooks/CI 消费同一清单；结构性例外四件（review-tier / review-brief / change-scope / gene-format）非平跑，机制见 gates.mts 头注。
