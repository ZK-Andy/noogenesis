...
- **演化发动机**——`engine/` 八命令（`select` / `propose` / `evaluate` / `solidify` / `pull` / `observe` / `capsule` / `mutation`，Node 零依赖）+ 验证白名单（`engine/gates.json`）+ 基因/Capsule/Mutation/事件协议。
...
├── engine/                 # 演化发动机（Node 零依赖八命令 + gates.json 白名单；pull = 基因库只读消费；observe = 观测输入面；capsule = 执行审计记录；mutation = 执行前意图声明；源码 TS，运行形态 = tsc dist）
