...
Related: 实施计划 [coding-enforcement-impl-plan](../../../../docs/research/coding-enforcement-impl-plan.md)（本批 = 批 1 轨道 A，procedure 单源）；能力层与档位纪律 [2026-09-08-b4-mount-wiring](../architecture/2026-09-08-b4-mount-wiring.md)（A4 决策形态 `additionalContexts` + 建议档）；判据单源 [code-standards](../../../../docs/method/code-standards.md) + [.oxlintrc.json](../../../../.oxlintrc.json)（c2 立项 [2026-09-08-c2-lint-enforcement](../architecture/2026-09-08-c2-lint-enforcement.md)）；问题池余项 [capsule-01-optimization-round](../../../../docs/research/capsule-01-optimization-round.md) §2.1「规范事前接入」

## Problem

- 编码规范已「成文 + 机器拦」（[code-standards](../../../../docs/method/code-standards.md) + c2 的 `lint` / `export-docs` 两闸），但机器面全部在 **git 边界之后**：pre-commit 只覆盖暂存面、pre-push/CI 在推送面。写码当轮模型拿不到任何规范反馈——违反规范要到提交时才可见，AI 写码循环内无纠正回路（问题池「规范事前接入」条余项）。
...
1. **策略工厂与行为合同**：`createLintFeedbackPolicies(config: RepoRootConfig, deps?: { runLint?: RunLint; warn?: (message: string) => void }): { toolPost: ToolPostPolicy }`。逐条（实现与 selftest 夹具一一对应）：
   ...
   | 5 | 扩展名 ∉ `{.ts, .mts}` | `void` |
   | 6 | 仓根缺 `.oxlintrc.json` 或 `node_modules/oxlint/bin/oxlint` | `void` + 每会话至多一条 warn（离线降级，同 bank-pull 纪律） |
...
- 延迟实测（n=5，`node oxlint --config .oxlintrc.json -f json <单文件>`，2026-09-08 本机，【探索性】）：0.07–0.12s/次；实施计划估的 30–60ms 偏低（进程启动主导），仍属可接受写码往返开销。
