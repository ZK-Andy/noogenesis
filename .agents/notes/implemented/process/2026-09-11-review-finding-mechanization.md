...
Related: 评审契约 [review](../../../../docs/method/review.md)（findings 形态与三路收窄）、阶段与归集面 [撤除 ADR](2026-09-12-absorption-async-round.md) + 主链路 [feature-flow](../../../workflows/feature-flow.md) §4.6（挂载点）、收尾检查单 [session-close](../../../workflows/session-close.md)（findings 去向对账行）、门槛判据 [anti-overdesign](../../../../docs/method/anti-overdesign.md)；落地件 [verify-command-surface](../../../../scripts/verify-command-surface.mts)、[verify-package-invariants](../../../../scripts/verify-package-invariants.mts)、[verify-secrets](../../../../scripts/verify-secrets.mts)；写作面 [doc-standards](../../../../docs/method/doc-standards.md) 铁律 6。
...
**1. 机械化判据 = 判据稳定（形状可枚举、不看语义）且噪声实测过关即立**；噪声率高才不立（门禁自身是维护面且会带缺陷：[verify-secrets](../../../../scripts/verify-secrets.mts) 的 URL 族误报、本件 Decision 3 的变更史词面闸实测噪声 ≈95%）。单次代价高者（凭据泄漏 / 发布事故 / 数据丢失）不受噪声实测约束。阶段与归集面的位置见 [撤除 ADR](2026-09-12-absorption-async-round.md)；门槛条文仍以本件为单源。
...
- **三类复发不再进评审**：命令面漂移、README 版本漂移、凭据模式表的双向覆盖（后者的夹具面）。后续机械化件（如 [适配层宿主服务读取面闸](2026-09-13-host-service-reads-gate.md)）的家 = 各自批次 ADR，本件只登记这批三类。
