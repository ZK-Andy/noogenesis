...
**1. 机械化判据 = 判据稳定（形状可枚举、不看语义）且噪声实测过关即立**；噪声率高才不立（门禁自身是维护面且会带缺陷：[verify-secrets](../../../../scripts/verify-secrets.mts) 的 URL 族误报、本件 Decision 3 的变更史词面闸实测噪声 ≈95%）。单次代价高者（凭据泄漏 / 发布事故 / 数据丢失）不受噪声实测约束。阶段位置单源 = [吸收阶段 ADR](2026-09-11-absorption-stage.md)。
