# Agent Note: 协作层重建批次方案——lefthook 钩子框架、0.2.0 切换断点

Status: implemented

Related: [2026-09-06-framework-rebuild-charter](2026-09-06-framework-rebuild-charter.md)（charter，本 ADR 为其协作层重建的批次方案）；[2026-09-08-b0-framework-structure](2026-09-08-b0-framework-structure.md)（B0 批拆解与 TS 钉形）；蓝图对账单源 [framework-rebuild-blueprint](../../../../docs/research/framework-rebuild-blueprint.md) §10 C1–C15

## Problem

charter（[2026-09-06-framework-rebuild-charter](2026-09-06-framework-rebuild-charter.md)，implemented）拍板协作层重建以蓝图 [framework-rebuild-blueprint](../../../../docs/research/framework-rebuild-blueprint.md) §10 C1–C15 为验收单源，立项时实现轮未定：批次切分、钩子框架选型、切换断点版本。立项时机器层实盘（2026-09-06）：
...
- **蓝图未定项三件**：`mdref.py` 归宿——实读已定：它是 `verify-md-links`/`verify-skill-format` 的共享链接原语库（import 消费，非独立工具），归宿 = 迁 TS 共享模块 `mdref.mts`（B1 钉形修正：type:commonjs 下裸 .ts 为 CJS 分类装不下 import），不淘汰；`setup-hooks.sh` 去向——拍板 = lefthook 内建 postinstall 取代（见 Decision）；`pre-push-selftest.sh` 四态 e2e——TS 重建为独立 e2e 件（运行形态随 B0 拍板：node 原生 type stripping 直跑，tsx 落败，见 [2026-09-08-b0-framework-structure](2026-09-08-b0-framework-structure.md)）。
...
| 批 | 内容 | 蓝图对账 |
|---|---|---|
| **B0 框架结构面批** | 四件框架结构件：子树 AGENTS.md 五件布点（C11）/ 教训层形态（postmortem 规则入门禁，C12）/ archived-notes 校验件（决策记忆层增量）/ 技能 references/ 形态规则（不实拆——noo-doc-standards 377 词（verify-doc-budgets 口径实测），无携带需求）；新门禁直接 TS——node ≥22.18 原生 type stripping 直跑 .mts（tsx 落败，零 devDependency；tsc/tsconfig 归 B1），最小切片 = CI 钉 node 22 | C11 / C12 / §2 增量 / §3 形态 |
...
**批次纪律**：每批触碰 `scripts/**` 门禁判据 = FULL 三审（tier 机械触发）；并存期（B1–B4）旧机器件保持权威执行面，TS 件以双跑对账自证；B5 单批切换后旧机器件全量删除（charter 口径）。
...
- **验收与对账**：每批 FULL 三审采纳收口 + 蓝图对账表对应行转绿 + journal 月卷「按蓝图判据」标注（事件轨 kind 封闭集只收 gene 事件）；B5 后 C1–C15 全绿（C6 基线数值实测回填）、旧机器件（py/sh/旧 js 源）零残留、`noogenesis@0.2.0` 已发版（tag 随钩子）；每批核 C9 = 6 基因 + 事件轨文件 diff 为零，gates.json 条目集与门禁名至 B5 前不变。
- **并存期风险**：五批的双机器并存窗口内权威归属可漂移——缓解 = 旧件保持权威 + 每批 journal 月卷标注 + B5 一次收口。
- **依赖边界**：lefthook 是第三方 devDependency——D1 约束 engine 运行时面，工具链面已由 TS 拍板豁免；engine 零依赖纪律不变。
- **对账工作量与残件**：py→TS 行为恒等逐件实证（含 self-test 夹具同迁）；对账脚本为临时代码，不入门禁、B5 删除。
- **mdref.mts 迁移面**：链接/锚点正则与 slugify 语义逐条对齐（两消费方 self-test 同迁覆盖）。
