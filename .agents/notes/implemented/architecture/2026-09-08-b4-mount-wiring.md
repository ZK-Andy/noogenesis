...
Erratum: 2026-09-08 — 撤回 Decision 3（M1）/Decision 4 后半（A3 触摸记录）/Decision 5（M3）/A6 投影位/A8 `session.append` 落点，以及 Consequences「风险面」首条的「持久化容忍度实机验证 ✅」结论：重验只覆盖写路径落盘，宿主读路径（会话历史 observe）对词汇表外且未标 `ignorable` 的事件类型 fail-closed 拒解释整份日志（装机 `Session.append` 无 ignorable 参数位——「feedback/record 先例支持自定义 kind」亦证伪：该类型在词汇表内且 data 即 `{ text: string }`，`tool/code-dispatch-start` 先例是仓内包构建期合入词汇表，下游插件无同款通道）。保留 A2 开场地图、A3/A4/A5 能力层与 C15 评估；Consequences「WeakMap 会话键生命周期」行的「显式清态 = `session/disposed`」drain 陈述一并撤回（drain 面随撤除批删除，残留状态仅 A2 mapShown 布尔，WeakMap GC 自清兜底）。撤除拍板与机制链单源：[2026-09-08-a8-session-record-projection-removal](2026-09-08-a8-session-record-projection-removal.md)。

Related: [2026-09-06-collab-rebuild-impl](2026-09-06-collab-rebuild-impl.md)（五批立项，本 ADR 为 B4 批实现轮）；[2026-09-06-m2-adapter-wiring](../../implemented/architecture/2026-09-06-m2-adapter-wiring.md)（防火墙规则 2 的原拍板单源，本 ADR Decision 2 部分修订其允许集）；[2026-09-08-b3-hooks-install](../../implemented/architecture/2026-09-08-b3-hooks-install.md)（前批，dist 自测面）；蓝图对账单源 [framework-rebuild-blueprint](../../../../docs/research/framework-rebuild-blueprint.md) §7 挂载面全清单 + C7/C8/C15；评审记录件升格判据 [review](../../../../docs/method/review.md) §5；**A4 block 升格批 [2026-09-09-lint-block-and-staged-hook](2026-09-09-lint-block-and-staged-hook.md)**（本件「A4 block 首批不使用」档位选择被该批升格取代——A4 现行语义以 09-09 件为准）
...
- **风险面**：
- **session.append 自定义 kind 的宿主兼容面**：`feedback/record` 先例支持自定义 kind，但持久化插件对未知 kind 的容忍度未经实机验证——本批 smoke 为脱宿主冒烟，实机验证随下一次桌面重验（todos B 类惯例）；失败面 = 记录缺席（降级纪律兜底），无阻断风险。（实机验证 2026-09-08 ✅：`noogenesis/review-surface` 以一等事件类型落宿主 session 持久层、turn 级增量与对照探针精确对账——journal 2026-09 卷重验批在案；同批实锤 M1/M2 A3 观测面 `exec.args` 字段误读缺陷，修复 ADR [2026-09-08-mount-exec-arguments-field](../bug-fix/2026-09-08-mount-exec-arguments-field.md)。）
