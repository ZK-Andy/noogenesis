...
2. §4 的判不立条目缺外部证据面：import 环检测闸只有单一样本（2026-09-10，n=46），上帝类闸的停档理由只有本仓 2026-09-08 分布（n=38）、无上游同款立场可比。
...
**对照结论：六条无一条被实质违背**；第 2 条形态不同——`adapters` → `engine` 是进程边界（spawn CLI + stdout 文本 + 退出码三档）而非同进程模块调用，但两边的守层手段同为语义档：本仓跨族互斥是 `[R]`（R1b；R1a 的防火墙断言只覆盖宿主依赖允许集），TS 靠 review 约定；实测 0 跨族边是该档当下的证据面。

### 2. 第二样本（读数家 = [architecture-standards §4](../../../../docs/method/architecture-standards.md)）

2026-09-15 复跑相对 import 静态建图（population 与逐语句计数口径 = §4 节首，含 `adapters/hermes`；两轮读数 = §4 import 环检测闸条）：环 = 0、跨族边 = 0、全部可解析，与 2026-09-10 同向。规模（`wc -l`）：engine 15 件 2,944 行、adapters 20 件 4,833 行（dsh 17 + hermes 3）、scripts 29 件 10,288 行。

- 宿主依赖面实测同 R1a：值 import 仅 `adapters/dsh/index.mts`（`dsh-tools` + `dsh-llm`）；`engine/` 只有 node 内建（`fs` / `path` / `crypto` / `child_process` / `os`）。
- **【探索性】：基线样本 n=2**，不作趋势判断；两个样本同向支持 §4 的 import 环检测闸（判不立）与层方向 lint（未立）维持现值。
...
- **b. 三个 `selftest`（engine + adapters/dsh + adapters/hermes）随 `dist/` 进发布面**——维持现状。engine 侧是设计（README 让自托管用户跑 `node dist/engine/bin.js self-test`，自测即可复算证据面）；adapters 两件的消费者是仓内开发者与 pre-push / CI。**触发**：包体积或消费者困惑成为具名问题（届时评估 build 侧排除面，先核 dist 自锚定路径的耦合）。
...
- **解析面清单**：适配层唯一的**引擎 stdout 字段解析器** = `hitsSectionText`（吃 select 输出）；`commands.mts` 的 `list`（`trimEnd` + 追加 staging 行）/ `verify`（`trim` + stderr 兜底）与 `tools.mts` 三工具都只做空白修剪与整段回显、不解析字段（同层的 lint / export-docs 反馈面解析的是 oxlint 与仓内判据件的 stdout，非引擎输出）。
...
- **为跨族文本合同立共享断言 / 纪律件**：落败——适配层唯一的引擎 stdout 字段解析器，其依赖形态已由真实引擎实跑断言覆盖（见 Decision 4），失配通道不存在；触发条件已具名，届时再立。

## Consequences

- **单源与指针**：读数家 = [architecture-standards](../../../../docs/method/architecture-standards.md) §4（口径 + 两轮读数 + 上游先例指针）；本笔记 = 对照结论与三处差异裁决的家。
- **零机器面变更**：无新命令、无 schema / 门禁清单 / 事件键集 / 发布面变更，无新夹具。
- **相邻决策不重开**：charter Decision 2 的基准范围只扩不缩；§4 各条判词（三条判不立 + 层方向 lint 未立 + project references 不现在做）与触发原样维持，本次仅补第二样本与上游先例；R1a / R1b / R6 口径不变。
