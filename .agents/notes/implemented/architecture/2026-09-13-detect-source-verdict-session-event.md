...
Related: 批次表 [行 11 与「需显式重拍的决策面」](../../proposed/architecture/2026-09-13-feature-completion-backlog.md) · [P1 骨架 D2](2026-09-05-p1-engine-skeleton.md) · [M2 适配层](2026-09-06-m2-adapter-wiring.md)（Detect 显式不做）· 主设计 [§6 生命周期 / §4.2 事件面](../../../../docs/research/dsh-swarm-evolution-framework-design.md) · 蓝图 [§7 挂载面表 / §9 不做清单](../../../../docs/research/framework-rebuild-blueprint.md) · [A8 撤除批](2026-09-08-a8-session-record-projection-removal.md) · [B4 挂载面 ADR](2026-09-08-b4-mount-wiring.md)（M1 HERO 答案）· 宿主包读面 `@deepseek-ai/dsh-session@0.1.5-rc.2`
...
- 主设计 §4.2 给 Session 事件面的语义 = 轨迹信号 / 压缩边界 / 固化落盘；据此四个候选失败面逐面过 HERO 两问（检测到什么具体失败 → 真出现后下一步做什么不同）：
  ...
  2. **压缩边界**（surface 折叠后重注基座）：A1 常驻基座按 system-prompt 有序节渲染，A2 开场地图经 pre-step 瀑布的 messages 面进入本步（user/message 面）——本仓零「折叠后失守」观察实例；重注的触发条件（何时算失守）本身无判据，为未观测面加 listener 即防过度设计所禁的投机建设。
  3. **循环重复**（turn/step 计数 + `tool/call` 名序列）：即 repeat-tool-reminder 形态，蓝图 §7 已裁「首批不含，逐件评估再挂」、§9 不做清单在案；本仓无长挂调用场景。
  4. **固化落盘**（`session/flush` / `session/end-seed` 触发 solidify）：同一失败面（会话结束盘点入档候选）已由 `agent/disposed` 单次终态边界承接（M2 Decision M2-3；flush 可在会话中途多次发生，M2 Alternatives 已否该触发点）。
...
5. **封条关系**：本件即批次 3 的 D2 重拍件；行 12–14 在其站立规则下直接开工，行 15（情境按需注入）仍受「自动 Detect 默认关」约束、须自身过 HERO 另案。

## Alternatives considered

- **判接（订阅 `session/event`，从事件流派生信号喂 select）**：落败——HERO 第一问无具名答案；派生器只剩 prose 分类（B4 已判不可判）与 A3/A4 已见面的复算两条路，且每事件回调抬高调用频次（M3 信号），代价与收益不成比例。
...
- [P1 骨架](2026-09-05-p1-engine-skeleton.md) D2 现值同步：自动扫描固定面的站立规则 = 默认关 + 逐源门槛，本件为单源。
- 连带 M3 重议：自动 Detect 的调用频次面随本件维持关而零变化；重开本源（T1–T3 任一）时按 M2 Consequences 同步评 M3。
- 复算口径 = 宿主包读面（`@deepseek-ai/dsh-session` 的 `Events` 声明与 `Session.append` 合同）+ 本仓 `adapters/dsh/mount.mts` 现态订阅面（零 `session/event`）。
