...
- **宿主组合面分两层，均原生在场**：profile 层 = `dsh.profile.bundles` 按序叠加的 plugin-bundle patch 层（机制读数单源 = [批次 6 裁决 ADR](2026-09-14-batch6-composition-meta-verdict.md) Problem；落点 `$DSH_HOME/profiles/` 在会话工作区之外）；agent preset 层 = `@deepseek-ai/dsh-agent-presets` 的每会话组合——一个 preset 目录（`agent.cordis.yml` + `preset.yml`）选定该会话的 tools / prompt 节 / skills，shipped 集为 `standard` / `minimal` / `ptc` / `cordis`，用户根 `<dshHome>/.agent-presets`；本会话卷头 `agentPreset: "ptc"`。
...
- **T3**：宿主给出插件侧 profile 组合 / 挂载服务面（行 31 原文；当前实测只有 CLI + 目录面——preset 发现面 = 部署配置 `roots` + copy-only 用户根，无插件侧注册 API）。
...
- **我方 bundle patch 追加一条 `dsh-agent-presets` row 配 `roots`**：落败——该服务已在宿主组合的 root realm 发布，第二条 row 会重复发布同名服务；且这是干预宿主包配置面，不是本包职责。
