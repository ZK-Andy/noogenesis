...
- **无第二个地址消费者**：跨机交换（[行 21/22](2026-09-13-feature-completion-backlog.md)）为零；[manifest.json](../../../../manifest.json) 是基因检索索引（`{ref,path,summary,signals}`），由 `verify-manifest` 与 `genes/` 对账——无地址需求；cookbook → 基因的批量 / 自动转化判不立（[序 40 裁决](2026-09-14-pitfall-atomization-verdict.md)）；引擎零 LLM。
