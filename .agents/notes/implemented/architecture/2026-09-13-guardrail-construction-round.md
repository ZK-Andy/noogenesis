...
- **读数口径**：每会话至多一行 `noogenesis token baseline reading: session=<id> surfaceTokens=<n> (whole hosted surface, host heuristic estimate; observation only)`，自**会话第二步**起取；`<id>` = `session.id`，非字符串写 `?`（读数行与两条降级 warn 行同带；复验判据要能从落盘单文件复算「每会话恰一行」与「该会话恰一条 warn」）。读数 = 宿主 token-meter 对当前 session surface（durable log 折叠面 = system / user / assistant / tool-result）的启发式估值，in-history 形态下含宿主 persona 与 AGENTS.md 注入；它是点时刻快照，不是本插件注入面真值，不进任何判据。**观察面 = `<DSH_HOME>/logs/noogenesis.log`**——0.2.7 装机首跑实测原「宿主日志」面无此通道；通道、证据与备选 = [插件日志落盘 ADR](../bug-fix/2026-09-16-plugin-log-sink.md)。未覆盖面（显式接受）：单步会话零读数。
