...
本 ADR **部分取代** [2026-09-06-m2-adapter-wiring](2026-09-06-m2-adapter-wiring.md) 的两处决定面——M1 插件形态的包名拍板（裸名 → 宿主后缀）与耦合防火墙规则 3（三级回退链 → 四级链）；M2 其余拍板（插件壳形态 / 接线点 / spawn 单合同）继续有效；M4 技能分发的**分发半边**已由 [2026-09-06-skills-ride-bank](2026-09-06-skills-ride-bank.md) 部分取代（gene→skill 渲染语义仍随贡献开放轮）。
