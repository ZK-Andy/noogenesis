...
3. **断言面落 `host-api-contract.mts`：服务键 `tokenMeter` + `TokenMeasurement.surfaceTokens` 为 `number` + `measure` 返回 `TokenMeasurement`。** 断言面逐条对应本层消费点：`surfaceTokens` 是运行期形状闸唯一读取的字段，`totalTokens` 等其余字段本层零消费，不入断言（否则宿主重命名零消费字段会让 `tsc` 红而插件行为零变化）。运行期形状闸保留——类型面盖声明漂移（宿主改名/换型即 `tsc` 红），运行期面盖真实返回值与缺席降级，两者互补不是替代。
...
- **升代实测**：升代前 `node_modules/@deepseek-ai/` 的 `dsh-*` 共 13 件、全 `0.1.0-rc.8`；升代后 `dsh-*` 共 20 件、全 `0.1.5-rc.2`——新增 7 件 = `dsh-token-meter` 本身 + 其 peer 面拉入的 `dsh-commands` / `dsh-compaction` / `dsh-llm-retry` / `dsh-session-projection` / `dsh-util-crypto` / `dsh-util-values`；lock 内零 `0.1.0-rc` 残留。`cordis@4.0.2` / `cosmokit@1.8.3` / `schemastery@3.18.2` 为独立版本线，不属该代。
