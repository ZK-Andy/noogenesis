...
- 断言面与消费点一一对应：事件键（`index.mts` 六个 `ctx.on`）、策略件入参形状（`ToolExecution` / `ToolExecutionResult` extends `mount.mts` 窄类型）、决策判别式（allow/deny/ask、block/context）、工厂形状（`defineTool` 返回、`createUserMessage` 入参）。
- 手法 = `type Assert<T extends true> = T` + 具名 `type _X = Assert<…>`；上游漂移即 `tsc` 红。

### D2 窄类型收窄修正（D1 抓出的唯一不成立项）

- `mount.mts` 的 `ToolExecLike.arguments` 由 `Record<string, unknown>` 改为 `unknown`——宿主合同即 `unknown`，本层声称对象是不实收窄；读取面（`lint-feedback.mts`）改为先判 `typeof === "object" && !== null` 再取键。行为不变（原 `?.` 对非对象同样得 `undefined`）。
...
- adapter selftest 增加**源码面**断言：`adapters/dsh/*.mts` 的 `@deepseek-ai/*` import 必须是 `import type` 且文件在闭集内（补上「dist 扫描看不见源码 import」的盲区）。
...
- **只断 `ToolExecution.arguments` 键存在，不做 assignability 断言**：落败——键存在盖不住「本层窄类型与宿主形状不符」的漂移，而 D2 抓出的正是这类真实缺陷。
