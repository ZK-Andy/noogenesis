...
3. **dist 布局**：`tsconfig.build.json`（nodenext、strict + noUncheckedIndexedAccess、rootDir=`.`、outDir=`dist`、零 declaration）→ `dist/engine/*.js` + `dist/adapters/dsh/*.mjs`，相对路径与源一一对应；`npm run build` 脚本 = `tsc -p tsconfig.build.json` + `dist/engine/gates.json` 资产拷贝步（见点 7）。B1 根 tsconfig（noEmit typecheck）include 追加 `engine/**/*.ts` + `adapters/dsh/**/*.mts`，`ts-typecheck` 白名单闸名与条目不变。
...
- **采用面（已落地，2026-09-08）**：engine 10 件 .ts + adapters 9 件 .mts + `tsconfig.build.json` + `npm run build`（tsc + gates.json 资产拷贝）+ 根 tsconfig include 扩展 + package.json（files/main/exports→dist）+ validate.yml（build 步 + TS 双列）+ `scripts/reconcile-b2.mts` 临时代件 + selftest 判据面双份同改（js 与 .mts）。**对账结果：reconcile-b2 四件零 diff（engine self-test/select/propose/fail-closed 用法面 + adapter self-test 双跑）+ C9 资产零漂移；engine self-test 双面 stdout 69 行逐字节全等，adapter self-test 双面 53 组逐字节全等；npm pack 试跑包内 29 文件含 dist/engine/gates.json，解包烟测 select exit 0 / evaluate exit 1 / 非 git cwd exit 2 三档在位。**
- **评审面（R1/R2/R3 全采纳收口）**：R1 0B/1S（bin.ts pull 不可达 guard 可合并——延后随 B5/下次触碰，记 HANDOFF-todos C 条）；R2 0B/3S（files 含 selftest.mjs——拒绝，0.1.x 旧白名单同形态，收窄归 B5 发布面；ADR files 措辞 `dist/**`→`dist/`——本批已对齐；guard 风格——并入 R1 延后项）；R3 0B/5S（立项 ADR 11→10 件随收口同步、点 3 拷贝步补写、scripts/AGENTS 指针改相对链接、files 措辞、Related 链接文字——全部本批收口处理）。
- **风险面**：TS strict + noUncheckedIndexedAccess 对 10+9 件的类型收紧可能暴露隐性 bug——暴露即修并记 journal，不改行为；双模块 import 形态两套并存（scripts `.mts` 直跑 vs engine/adapters dist 形态）——scripts/AGENTS 已立分野规则防新件走错形态；dist 为构建产物不入 git，CI/发布每次现建。
- **对账口径**：允许差异 = 程序名归一（spawn/报错文案中 `engine/bin.js` vs `dist/engine/bin.js` 路径段）+ self-test 夹具随机 mkdtemp 路径；除此之外逐字节全等。C9 每批核：`git diff genes/ events/` 为零 + engine/adapter self-test（权威面与 TS 面双双绿）。

- **验收（全过）**：engine 10 件 + adapters 9 件 TS 化落地，`tsc --noEmit`（全仓含新 include）零错 + `npm run build` 产物 `dist/` 自证 + 双跑对账零 diff（reconcile-b2）+ 权威面/TS 面自测双绿 + C9 零 diff + C1 部分/C3/C10 对账转绿；FULL 三审采纳后转 implemented，journal 月卷落「本批按蓝图判据」标注。
