...
- 判据零新增：与 `lint` 闸消费同一 `.oxlintrc.json`。`export-docs` 在本批不入在环面；该排除由 2026-09-10 扩面批解除（实读判据 = 单文件 AST 判定，可入在环）——[扩面批](./2026-09-10-export-docs-inloop.md)。
