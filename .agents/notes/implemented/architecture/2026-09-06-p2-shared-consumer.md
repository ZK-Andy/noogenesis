...
- **D7 · pull 触发 = adapter 惰性一次**（触发点面已被 [2026-09-06-bank-pull-session-trigger](../../proposed/bug-fix/2026-09-06-bank-pull-session-trigger.md) 部分取代：装载期仅 repoRoot 显式可知时触发，否则首个 `agent/created` 以会话工作区锚定——装载期 cwd 兜底在桌面部署下落错目录）：adapter config 新增 `geneBankUrl`（可选）；在场时插件装载触发一次 `pull`（逐仓 in-flight 去重，拉取完成后不释放——每实例每仓至多一次，刷新 = 重跑 pull 或重启），失败仅 warn 降级离线不阻塞会话——拉取是便利面不是正确性面（本仓基因始终在场）。引擎 `pull` 命令同时保留手动入口。被否的纯手动触发落败于「配置了却忘 pull」的静默陈旧。
