...
**D3 · P2 实现轮范围 = 共享客户端 pull 消费 + manifest 最小检索索引 + 跨仓 genes/ 合并扫描。** 后置（逐项有主）：capsules/ 原语（P1 最小闭环拍板后置）、schema 扩字段与 YAML 迁移（随贡献开放，js-yaml 例外权 open 单源在骨架 ADR）、观测透镜（两稿分歧收口 = 后置，随共享层设计稿 P3 口径；主设计 §12 P2 行的「观测透镜」改读后置）、gene→skill 渲染（M4 语义，仍随贡献开放轮；其中**技能分发**半边已提前收口为「技能随库分发」，见 [2026-09-06-skills-ride-bank](2026-09-06-skills-ride-bank.md)）、Genesis 世界观基因入档（内容策展，随首批外部基因轮）。
...
- **D7 · pull 触发 = adapter 惰性一次**（触发点与缺省面已被 [2026-09-06-bank-pull-session-trigger](../bug-fix/2026-09-06-bank-pull-session-trigger.md) 部分取代：`geneBankUrl` 缺省官方库（`false` 显式禁用），不再是「可选、未配即离线」；触发点 = 装载期仅 repoRoot 显式可知时触发，否则首个 `agent/created` 以会话工作区锚定——装载期 cwd 兜底在桌面部署下落错目录）：adapter config 新增 `geneBankUrl`；插件触发一次 `pull`（逐仓 in-flight 去重，拉取完成后不释放——每实例每仓至多一次，刷新 = 重跑 pull 或重启），失败仅 warn 降级离线不阻塞会话——拉取是便利面不是正确性面（本仓基因始终在场）。引擎 `pull` 命令同时保留手动入口。被否的纯手动触发落败于「配置了却忘 pull」的静默陈旧。
