...
- **第二宿主在场**：`hermes` CLI（`~/.hermes/hermes-agent` 源码 + `website/docs/user-guide/features/*.md`）。四个 hook 系统；其中 **shell hooks** = `~/.hermes/config.yaml` 的 `hooks:` 块 + 子进程 stdin/stdout JSON 协议（`shlex.split`，`shell=False`，命令语言任意），可在 `pre_tool_call` 阻断、在 `pre_llm_call` 注入上下文；`hermes hooks test <event> [--payload-file]` 可离线发射、打印 wire shape。
...
- **不做清单核对**（蓝图 §9）：已判不做三条与本刀的关系——用户全局 `AGENTS.md`/`CLAUDE.md` 副本（本刀不写全局指令文件）、claude-code/codex shell-hook 兼容层（本刀只发 Hermes 原生 shape，不做上游方言桥）、技能分发机制建设（本刀不自建分发：`.agents/skills` 是宿主原生路径，信任一步归用户）。
