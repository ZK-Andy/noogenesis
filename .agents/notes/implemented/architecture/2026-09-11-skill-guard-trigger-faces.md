...
Related: 机制立项 [2026-09-10-m1-guard-anti-overdesign](2026-09-10-m1-guard-anti-overdesign.md)（守卫三件套 Proposal 1②——本件扩其触发面，决定与档位不变）；语义面归属 [session-close](../../../workflows/session-close.md) §2；盲区取证 [journal 2026-09 卷](../../../../journal/2026-09.md)（2026-09-11 两批的收尾对账 + 本件宿主日志实测）。
...
实测（宿主日志实取，n=19 个 Noogenesis 会话，2026-09-09..09-11，单机单仓样本，【探索性】）：现存两条条目实际发行 advice 的会话 = 2（去重 3 条：docs→doc-standards ×2、`.agents/notes`→archive-agent-notes ×1）；19 个会话全部用 `write` / `edit` 写过 `.md`，其中 `noo-prose-standard` 在首个 `.md` 写之前已载 = 3；15 个会话发出过真实 `git push`，其中 `noo-pre-push-checks` 已在载 = 4。两处漏项在样本内触发面为零，而对应任务普遍发生。守卫生效后的两次漏项（`c231ce0c` 机械化批、`ef23170c` 吸收阶段批）宿主日志均为零 advice。

## Decision

**守卫②的触发面从「路径前缀 × 技能」扩为「三类匹配 × 技能」，并把 bash 中介写纳入写码目标。**

1. **条目形状**：`config.skillGuards` 条目 = `{ kind, pattern, skill }`，`kind ∈ { path, suffix, command }`——`path` = 写码目标路径的 POSIX 相对目录前缀；`suffix` = 目标路径后缀（扩展名，如 `.md`）；`command` = bash 命令文本的正则。旧形态 `{ path, skill }` 拒收（配置 schema 变更，fail-closed 指名字段）。
2. **写码目标两条通道**：`write` / `edit` 的 `file_path`，以及 bash 命令里的 `>` / `>>` / `tee` 目标（按会话 cwd 解析，出仓即弃）；两类通道同样参与 `path` / `suffix` 匹配。脚本体内的写（python heredoc 的 `sub(path, …)`、`sed -i` 等）不可枚举，留作残余边界（见 Consequences）。
...
- **只立行动区一条**（用户提供的二选一）：落败——纪律条文已两次失效，待办条没有触发时刻，与 M1 既定判据（触发交给机器、服从留给自觉）反向；待办条无法把「写 .md 的那一刻」或「push 前的那一刻」变成信号。
...
- **噪声上界由触发器给**：每会话每技能至多一行，不随写次数增长。样本内投影发行率（【探索性】n=19）：`.md` 面 16 个会话各 1 行（已载 3 个例外）、`git push` 面 15 个有 push 的会话中 11 个各 1 行（已载 4 个例外）。
- **残余边界**：脚本体内的写不可见（机械化批实例在案）；命令内 `cd` 改目录的复合命令按会话 cwd 解析——保守面（可能漏、实测无误报）。
