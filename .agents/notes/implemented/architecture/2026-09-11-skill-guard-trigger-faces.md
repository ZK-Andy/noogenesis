...
- **仓判定收紧**（技能面缺席时不发行）：缓议期结束——2026-09-12 实测出现误导（外仓提醒点名宿主目录里不存在的 `noo-*` 技能，agent 多耗两步去载失败）后落地，判据与实现 = [技能可达性门 ADR](../bug-fix/2026-09-12-bank-skill-provider-registration.md) Decision 2。
