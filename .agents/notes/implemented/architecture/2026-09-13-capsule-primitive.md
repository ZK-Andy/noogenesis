...
- **schema S2 重拍**：kind 封闭集三件 → 四件（增 `capsule.added`）；事件键集由固定七键改为**按 kind 条件化**——五键共通（`ts` / `actor` / `kind` / `outcome` / `evidence`）+ gene 面 `gene` / `gene_sha` 或 capsule 面 `capsule` / `capsule_sha`。S2 的落盘语义与复算分型不变。**现值随序 3 第三次重拍**（kind 五件、键集 = 必需键 + 按 kind 的允许可选键）：见 [Event 扩字段 ADR](2026-09-13-event-field-extension.md) E5。
