...
- **kind 封闭集（演化事件 ≠ 运行日志）**：v0 为 `gene.added` / `gene.updated` / `gene.retired` 三种，现为五件——`capsule.added`（[批次 1 序 1 ADR](2026-09-13-capsule-primitive.md) C5）与 `mutation.added`（[批次 1 序 2 ADR](2026-09-13-mutation-primitive.md) C4）随对应原语加入；键集 = 按 kind 条件化的**必需键** + 按 kind 的**允许可选键**（[批次 1 序 3 ADR](../../proposed/architecture/2026-09-13-event-field-extension.md) E5），本 ADR S2 其余口径不变。select/propose 运行不记（会话噪音，与 #18 单例判定约束的"常开确定性成本"同类——[cookbook 条目](../../../../docs/cookbook.md)）；evaluate 结果只在入档尝试时随 solidify 事件落一条（成功带证据，失败带拒因）。
- **落盘语义逐 kind 钉死**：`gene.added`/`gene.updated` = `genes/` 下文件新增/变更（工作树可复算）；`gene.retired` = 从 `genes/` 删除该文件（git 历史仍可溯），事件 `gene_sha` = 退役时最后内容 SHA；入档失败事件 = 候选未落 `genes/`，`gene_sha` = 被拒候选内容 SHA；`capsule.added` / `mutation.added` = `capsules/` / `mutations/` 下文件新增（工作树可复算，两者均 append-only）。
- **字段（P1 最小集）**：`ts`（ISO 时间）、`actor`、`kind`、`gene`（id）、`gene_sha`（内容寻址防篡改锚点）、`outcome`（ok | fail+拒因）、`evidence`（一行证据摘要）——capsule 面把 `gene`/`gene_sha` 换成 `capsule`/`capsule_sha`（[批次 1 序 1 ADR](2026-09-13-capsule-primitive.md) C5），mutation 面换成 `mutation`/`mutation_sha`（[批次 1 序 2 ADR](2026-09-13-mutation-primitive.md) C4）。主设计 §5.1 的 `mutation_id` / `capsule_id` / `env_fingerprint` 现为可选键，`validation_report_id` 仍不进（指向的"验证报告"对象尚无家）——逐条裁决与生产者面见[批次 1 序 3 ADR](../../proposed/architecture/2026-09-13-event-field-extension.md) E1–E4（口径：字段先于原语出现即死字段）。
...
- **校验**：不新增门禁——`verify-gene-format.mts` 覆盖 `genes/` + `capsules/` + `mutations/` + `events/`：行级 JSON、kind 封闭集、月卷内时间序；**复算规则分型**——工作树存在的 `genes/<id>.json` ↔ 其最新 added/updated 事件的 `gene_sha` 必须一致（复算），`capsules/<domain>/<id>.json` ↔ 其 `capsule.added` 事件的 `capsule_sha` 同理（[Capsule ADR](2026-09-13-capsule-primitive.md) C4），`mutations/<domain>/<id>.json` ↔ 其 `mutation.added` 事件的 `mutation_sha` 同理（[Mutation ADR](2026-09-13-mutation-primitive.md) C4）；fail/retired 事件只查结构/kind/时间序，**不作工作树复算**（内容经 git 历史复核）。可选键另判形状（`env_fingerprint`）与可解析（`mutation_id` / `capsule_id` 命中同名原语 id 集）——[批次 1 序 3 ADR](../../proposed/architecture/2026-09-13-event-field-extension.md) E6。仍单脚本 = 白名单外独立件，gate 数量的克制性守住。

### S3（2026-09-05，已拍板）：验证白名单、安全模型、元评测夹具

- **白名单载体 = `engine/gates.json` 版本化机器清单**：v0 内容 = 九门禁（无参门禁直接实例化；带上下文参数的 review-tier/review-brief/change-scope 以参数槽收录，取值仅由引擎从 git 事实注入，见骨架 ADR D4）。**P1 消费者 = 引擎 evaluate + `verify-gene-format.mts` 的条目脚本存在性校验**（防漂移）；hooks/CI 经 `scripts/gates.mts` 改引同一清单（2026-09-06 收口，[M2 ADR](2026-09-06-m2-adapter-wiring.md)；结构性例外见该脚本头注（B5 起单家 = gates.mts 头注））。与 AGENTS 质量门（[根 AGENTS.md](../../../../AGENTS.md) 人读面）的漂移由上述双机械闸兜底。
