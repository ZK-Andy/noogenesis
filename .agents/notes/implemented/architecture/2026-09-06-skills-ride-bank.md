...
- **交付更正（2026-09-12 实机实证；2026-09-13 修复批回填）**：本件的「外部装包 → pull 即得 7 技能」在装载顺序把 skills 服务推到 `apply()` 之后时不成立——注册改为可重试注入 `ctx.inject(["skills"], …)`，提醒面改为只点名本会话目录里实际可载的技能；机制、夹具与真机验收口径 = [2026-09-12-bank-skill-provider-registration](../bug-fix/2026-09-12-bank-skill-provider-registration.md)。
