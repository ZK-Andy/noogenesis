...
`.review-briefs/` 是 gitignored 的本地评审工件目录（在飞的简报在目录根，收口后进 `archive/`），`verify-md-links` 按仓库文档扫描它：简报里的相对链接以工件所在目录为解析基准，而「在飞用 `../.agents/...`、归档后须改 `../../.agents/...`」——归档步本身即必漏点（批次 8 序 40/序 41 各一次实测：归档后即报 missing target，而同简报自证行写 `md-links:0`）。工件不进 diff、不是仓库文档，校验它们只产生假红；自证读数与实跑 exit 脱节又把红读成绿。
