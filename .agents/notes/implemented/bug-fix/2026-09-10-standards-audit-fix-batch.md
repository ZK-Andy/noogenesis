...
6. **verify-postmortem-naming 排序不确定**：`localeCompare` 随运行环境 locale 漂移，而编号递增判据依赖该序；家族纪律明确钉码点序（change-scope/gen-manifest/verify-manifest 同口径）。
...
6. **postmortem 排序钉码点序**：`localeCompare` → 码点序比较器（对现有 ASCII 文件名零行为差，消除环境漂移面）。
...
- **行为面变化**：`propose --out` 缺值从静默成功改为 exit 2（CLI 合同面唯一变化，engine self-test 全过）；`evaluate` spawn 失败报告多一行根因；命中节故障期每会话至多一条新 warn；verify-adr-format 现在会拦 lifecycle 树外杂散笔记（本仓树现况零命中，真实扫描 OK）。
