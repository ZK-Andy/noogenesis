...
4. **模型面字符串语言无单源拍板**：基座节/命中节/子树地图/lint 拦回/工具 description/warn 实况全英文，注释面全中文——口径没有一次显式拍板，中英混排风险无主。
...
- **4**：拍板**模型可见字符串统一英文**（注释与 durable 文档仍中文）；口径单源归档 adapters/dsh/README「语义与失败模式」节。本项零代码变动——现状即英文，de facto 升格明文。
