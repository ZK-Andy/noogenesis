...
2. **runner**：`scripts/gates.mts`（沿用 py 单源发射器名——hooks/CI/引擎消费的是「gates」契约面；蓝图 §2 上游名 `run-gates.ts` 为描述性名，不搬）。蒸馏三能力（蓝图 §2）：`needs`（硬依赖，依赖失败则下游 skipped）/ `after`（只排先后不传染失败）/ fail-fast + 有界并行（默认并行度 = CPU 数，`--jobs` 可调含 1=串行）；运行前图校验拒绝重 id / 未知依赖 / 环（fail-closed exit 2）。CLI 同 py：`--list` / `--run` / `--self-test` / `--skip` / `--slot`（槽位替换语义同 py：任意 {{key}}、缺值 fail-closed、--list 缺值以 `<key>` 占位）。
