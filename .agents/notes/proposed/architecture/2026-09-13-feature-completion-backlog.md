...
- **未交付**：下方批次表余 32 项（序 1–14 已 done，批次 3 进行中）。
...
- **封条纪律**：批次 3 的 D2 重拍件已立（[裁决 ADR](../../implemented/architecture/2026-09-13-detect-source-verdict-session-event.md)：默认关 + 逐源门槛），行 11–14 四源已全部裁决（全判不立）、行 15 须自身过 HERO；批次 4 开工前必须先有独立的重拍 ADR。
