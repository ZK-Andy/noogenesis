...
- **技能使用守卫缺口**：7 个 `noo-*` 技能全自觉调用——「该不该用」（任务对口的技能没载）与「用了没有」（调用痕迹无处对账）均无任何机器面。B4 曾落纯记录件（[2026-09-08-b4-mount-wiring](../../implemented/architecture/2026-09-08-b4-mount-wiring.md) Decision 3：A3 观测 + A6 投影），随 [A8 撤除批](../../implemented/architecture/2026-09-08-a8-session-record-projection-removal.md)整体退役——宿主无 ignorable 写入口，投影路线不可复用；证据面回归宿主日志 `tool/result` 事件文本（既定口径）。
...
1. **技能守卫 = 三件套 v0**（触发交给机器，服从留给自觉）：
   - **① 技能路标**：A2 开场地图加一行「任务型 → 技能名」映射（动 `docs/**` → `noo-doc-standards`；评审收口 → `noo-code-review`；push 前 → `noo-pre-push-checks` 等）——常驻零自觉可见，调用仍自觉。
   - **② 触点提醒**：tool-pre 钩子（[B4 已证可用的挂载面](../../implemented/architecture/2026-09-08-b4-mount-wiring.md)）按**文件路径模式**判「本会话该载而未载」（写 `docs/**` 而未载 `noo-doc-standards`）→ `agent.inject` 一行 advice，**不阻断**（repeat-tool-reminder 同款哲学）；映射表进 config，条目按需增删。
   - **③ 事后对账**：session-close 流程卡加一步——grep 宿主日志 `tool/result` 技能调用痕迹，列出本会话用了什么；「该不该」的裁断留人/评审（A8 撤除后既定证据面）。
...
3. **实施分两批**，各自带评审与 ADR 翻转：守卫 = 代码批（adapters 挂载件 + A2 地图 + 流程卡）；防过度 = 文档批（根 AGENTS + method 新篇 + cases 索引）。本 ADR 随两批全部收口翻转 implemented（单批先收可先翻，本文留分批翻转批注）。
...
- 守卫三件在真实会话可观察：开场地图含技能路标行；写 `docs/**` 而未载对口技能时收到一行 advice（不阻断、可继续）；session-close 流程卡含对账步骤且跑过至少一次留痕。
- 根 AGENTS.md 含精简契约块且字数 ≤800；`docs/method/` 新篇存在且过 manifest 预算；cases 索引在 `docs/research/` 且不进任何常驻/懒加载面。
- 本 ADR 随实现批翻转 implemented，两批各自的 `Review` 证据行在案。
