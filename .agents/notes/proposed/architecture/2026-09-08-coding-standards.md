...
- 主设计 §10 内容域表「代码规范」列（编码约定、评审清单 → 可 lint/typecheck 机械校验）自 P1 起规划在案、至今未建；§10 亦列「注释规范半建」。
...
1. **新建 `docs/method/code-standards.md`（编码规范正文）**，**注释为主体**。结构（用户拍板 2026-09-08：注释为主要内容、规范正文完整）：
   - **注释规范（最大内容块，独立成文）**：公共 API 契约注释判据（prose-standard 现有判据收编 + 扩写）、何时写/不写注释、注释不写什么（推理转写/变更史/slop，承接 doc-standards 铁律与 CoT 治理）、JSDoc 档位声明；
   - **命名与结构约定**：标识符、文件组织、模块边界（判别式写法，可判定「是否违反」）；
   - **格式与风格**：当前零 lint 工具，先落可判别式约定，工具化升档排 c2（STOP 档 = 留评审/IDE 提示）；
   - **评审清单**：上述语义面（机器盖不住）条目进 `docs/method/review.md` §5 兜底清单；
...
- **oxlint 接入**（上游同款：oxlint 1.76 + oxlint-tsgolint 7.0，走 TS 面最顺）+ 公共 API 注释**机器强制**（如 `no-warning-comments` / JSDoc 档位升格）；进 gates.json + hooks/CI；规则一次调完。
...
- c1 完成：`docs/method/code-standards.md` 存在且入 doc-budgets manifest；条目全部带档位声明 + 编号 + 判别式；评审清单 §5 含该域兜底项；根 AGENTS 有链接；门禁全绿。
- c1 **不引入**任何新 devDependency、不改 gates.json 条目（零工具批）。
- c2 另行立项（proposed ADR 记录 oxlint 选型与注释强制档位），c1 完成后才启动。

## Consequences

- 编码/注释规范从「半建 + 散落」转「单源成文 + 档位声明」，agent 写码可查、评审有兜底。
- c1 是纯文档变更，需 follow docs 门禁（md-links/doc-budgets/adr-format/handoff）与评审（含前提：编码规范属方法论正文，落 docs/method/，非 research）。
