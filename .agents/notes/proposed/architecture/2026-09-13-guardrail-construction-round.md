...
- **判据落点**：`section.mts`（预算、单价、判据三者同件，字面量不跨文件重复）+ `config.mts`（装载期调用，违约即 fail-closed 拒载）+ `adapters/dsh/selftest.mts` 断言块（5 条：基线字符数钉住 / 缺省过判据 / 上界加一拒收 / 满行满摘要的最坏情形渲染仍在预算内 / 上游超长 summary 只经「库增长 + 行数调高」触达，配置闸拦下）。
- **勘误（预算数值口径）**：决定 1 原拟「命中节字符预算（缺省 2048）」。实现期实测该值与**已发布缺省 `maxIndexGenes=12`** 不相容——12 行 ×（`MAX_SUMMARY_CHARS` 160 + 行开销 16）= 2112 已超 2048，即插件会拿自己的缺省配置装载失败。改为**预算由已发布缺省推导**：`HITS_SECTION_CHAR_BUDGET = (缺省行数 + 1) × 每行成本 + 溢出行提示`（当前 = 13 × 176 + 64 = 2352，可配上界 13），并加模块装载期自洽断言（缺省必须过判据）——预算与缺省不能再各说各话。决定语义（字面预算为阻断面）不变，变的只是阈值的推导来源与数值。
- **判据覆盖面（如实记）**：基座节是常量（漂移由自测钉住 545 字符），命中节字面上界只随 `maxIndexGenes` 变化，故本判据覆盖「配置把行数调大」这一条路径；「基因库增长」路径由 `maxIndexGenes` 封顶**加`MAX_SUMMARY_CHARS` 逐行截断**覆盖——上游超长 summary 不放大注入量，只放大上界假设被突破的风险，而该假设由售出的行成本（含 16 字符行开销）兜住。
