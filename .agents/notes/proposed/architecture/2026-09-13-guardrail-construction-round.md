...
- **判据落点**：`section.mts`（预算、单价、上界、判据同件，字面量不跨文件重复）+ `config.mts`（装载期调用，违约即 fail-closed 拒载）+ `adapters/dsh/selftest.mts` 断言块（6 条：基线字符数钉住 / 缺省过判据 / 上界加一拒收 / 本仓全部基因 ref 不越 `MAX_HIT_REF_CHARS` / 上界行数 × 上界行宽的实渲染仍在预算内 / `validateConfig` 装载期拒收越界配置）。
- **评审收口（FULL 三审，2026-09-13）**：R1 0B/4S、R2 0B/4S，采纳 7 条（合并去重后）——上界与判据共用导出常量（原实现两处各推一遍公式，仅因余数巧合同值）、`HIT_LINE_OVERHEAD_CHARS` 收回模块私有并改为「具名上界 + 自测逐件核验真实基因 ref」、溢出提示行长度按实文改正并去掉与「缺省 + 一行」的重复购买、判据去掉与 `config.mts` 重复的正整数守卫（原诊断还会归因错）、删模块装载期断言与无因果的基因夹具。
- **勘误（预算数值口径）**：决定 1 原拟「命中节字符预算（缺省 2048）」。实现期实测该值与**已发布缺省 `maxIndexGenes=12`** 不相容——12 行 ×（摘要上界 161 + 行开销）已超 2048，即插件会拿自己的缺省配置装载失败。改为**预算由已发布缺省推导**：`HITS_SECTION_CHAR_BUDGET = 固定开销 + MAX_HIT_LINES × 每行成本`（`MAX_HIT_LINES` = 缺省 + 1；评审收口后当前值 = 73 + 13 × 228 = 3037，可配上界 13）。决定语义（字面预算为阻断面）不变，变的只是阈值的推导来源与数值。
