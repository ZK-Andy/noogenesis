...
1. **出账（主会话）**：逐条 findings 归口为四元组「类 / 本批证据 / 可机械判性 / 去向」；四出口逐条落且只落一家；发现的**类**记入 journal（复发计数用于形态与噪声判断）。
2. **呈报与确认（主会话 → 用户）〔关口〕**：吸收账呈用户；**未获确认前不得执行出口、不得进 §6 提交**。改判在关口内提出，不静默改账。
3. **逐出口执行（主会话）**：按账逐条执行四出口；执行中发现判据不成立（如噪声实测不过关）→ 记回账里改判。
4. **回执（主会话）**：每条出口的落点（文件 / 门禁 / 条目 / 卡片）随批汇报，作为 §8 收尾前置门的判据。
