...
- **四出口**（逐条 findings 落且只落一家）：
  - **机械校验**：可机械判（形状可枚举、不看语义）且判据稳定 → 落门禁 / 夹具 / 元断言；噪声实测纪律与门槛单源 = [发现机械化 ADR](../notes/implemented/process/2026-09-11-review-finding-mechanization.md)。
...
- 判据、门槛与首例单源 = [发现机械化 ADR](../notes/implemented/process/2026-09-11-review-finding-mechanization.md)；本阶段不新增评审代理。
