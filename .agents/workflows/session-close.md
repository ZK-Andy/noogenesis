...
3. **决策落档（结论只落 durable 四家）**：本轮拍板结论是否已进 ADR / `docs/cookbook.md` / README / AGENTS？没有则补。**findings 去向对账**：本会话跑过评审的，逐条结论是否已采纳 / 拒绝（契约单源 = [review.md](../../docs/method/review.md) §6）、框架级候选（可机械判的类 / 纪律漏项 / 踩坑）是否已各写一行进 [HANDOFF-evolution-pool](../../HANDOFF-evolution-pool.md)——步序单源 = [feature-flow](feature-flow.md) §4.6；演化动作随演化轮执行，不在本批次。**结论只落 durable 四家，不落 HANDOFF 滚动窗**——滚动窗不复述结论；凡待跨会话用的复现细节/取证点/判据，正文进 durable 家，滚动窗与待办区只留指针。
