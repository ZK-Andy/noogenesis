...
2. **技能使用对账（M1 守卫③）**：grep 本会话宿主日志 `tool/result` 事件文本中的技能调用痕迹，列出本会话实际载过哪些技能；「该载未载」的裁断不自动化——对照本会话任务型留人/评审判断（证据面 = 宿主日志，A8 撤除后既定口径；拍板单源 = [M1 立项 ADR](../notes/implemented/architecture/2026-09-10-m1-guard-anti-overdesign.md) Proposal 1③）。
