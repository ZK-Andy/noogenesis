...
3. **决策落档（结论只落 durable 四家）**：本轮拍板结论是否已进 ADR / `docs/cookbook.md` / README / AGENTS？没有则补。**机械化对账**：本会话跑过评审的，[发现机械化](feature-flow.md) §4.6 是否已执行——可机械判且复发 ≥2 次或单次代价高的类是否已落成判据/夹具（阈值单源 = [机械化 ADR](../notes/implemented/process/2026-09-11-review-finding-mechanization.md) Decision 1），未达门槛的类是否已记入 journal 复发计数（判据单源 = [机械化 ADR](../notes/implemented/process/2026-09-11-review-finding-mechanization.md)）。**结论只落 durable 四家，不落 HANDOFF 滚动窗**——滚动窗不复述结论；凡待跨会话用的复现细节/取证点/判据，正文进 durable 家，滚动窗与待办区只留指针。
