...
4. **Release 正文生成**：`node scripts/release/release-note.mts <前一tag> <版本> > RELEASE_BODY.md`——双语分节（新增功能/体验优化/问题修复/其他变更 × New Features/Improvements/Bug Fixes/Other Changes）+ 每条 `@提交者`（git author→login 映射）+ `Full Changelog` 尾行；复制进 GitHub Release（标记 Latest、非 draft）。
