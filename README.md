...
- **Evolution engine** — `engine/` exposes nine commands (`select` / `propose` / `evaluate` / `solidify` / `pull` / `observe` / `capsule` / `mutation` / `distill`) with zero Node dependencies, a validation whitelist (`engine/gates.json`), and the gene/capsule/mutation/event protocol.
...
├── engine/                 # evolution engine (zero-dependency nine commands + gates.json whitelist; pull = read-only bank consumption; observe = observation input face; capsule = execution audit records; mutation = pre-execution intent declarations; distill = failure-face digest + gene candidate staging; TS source, run form = tsc dist)
