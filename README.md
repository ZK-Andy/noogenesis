...
- **Evolution engine** — `engine/` exposes ten commands (`select` / `propose` / `evaluate` / `solidify` / `pull` / `observe` / `capsule` / `mutation` / `distill` / `list`) with zero Node dependencies, a validation whitelist (`engine/gates.json`), and the gene/capsule/mutation/event protocol.
- **DSH adapter** — this repository is also a DSH plugin package, [`noogenesis-dsh`](https://www.npmjs.com/package/noogenesis-dsh)@0.2.9 (AGPL-3.0). The adapter wires the engine into the session lifecycle via a single spawn CLI contract and carries the seven `noo-*` skills on the gene-bank cache; the provider registers through a retryable `ctx.inject(["skills"], …)` path, so a skills service that arrives after plugin load still lands the catalog ([ADR](.agents/notes/implemented/bug-fix/2026-09-12-bank-skill-provider-registration.md)).
...
├── engine/                 # evolution engine (zero-dependency ten commands + gates.json whitelist; pull = read-only bank consumption; observe = observation input face; capsule = execution audit records; mutation = pre-execution intent declarations; distill = failure-face digest + gene candidate staging; TS source, run form = tsc dist)
├── adapters/dsh/           # DSH adapter layer (plugin shell wiring: system-prompt section / noo_* tools / solidify confirmation / geneBankUrl lazy pull / bank-distributed skills / plugin log sink (`<DSH_HOME>/logs/noogenesis.log`) / five-point mount wiring A2–A6 with the A2 opening map (the A6 stop-time lane steers, see the [A6 ADR](.agents/notes/implemented/architecture/2026-09-13-a6-turn-stopping-mount.md); A8 record projection removed, see the [removal ADR](.agents/notes/implemented/architecture/2026-09-08-a8-session-record-projection-removal.md)); TS source since B2, npm package = tsc dist)
├── .noogenesis/           # evolution state face — tracked: genes/ events/ capsules/ mutations/ candidates/ manifest.json; ignored: genes-cache/ observations/
