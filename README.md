...
- **DSH adapter** — this repository is also a DSH plugin package, [`noogenesis-dsh`](https://www.npmjs.com/package/noogenesis-dsh)@0.2.7 (AGPL-3.0). The adapter wires the engine into the session lifecycle via a single spawn CLI contract and carries the seven `noo-*` skills on the gene-bank cache; the provider registers through a retryable `ctx.inject(["skills"], …)` path, so a skills service that arrives after plugin load still lands the catalog ([ADR](.agents/notes/implemented/bug-fix/2026-09-12-bank-skill-provider-registration.md)).
