...
- **Evolution engine** — `engine/` exposes eight commands (`select` / `propose` / `evaluate` / `solidify` / `pull` / `observe` / `capsule` / `mutation`) with zero Node dependencies, a validation whitelist (`engine/gates.json`), and the gene/capsule/mutation/event protocol.
...
├── engine/                 # evolution engine (zero-dependency eight commands + gates.json whitelist; pull = read-only bank consumption; observe = observation input face; capsule = execution audit records; mutation = pre-execution intent declarations; TS source, run form = tsc dist)
