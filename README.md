...
- **Evolution engine** — `engine/` exposes seven commands (`select` / `propose` / `evaluate` / `solidify` / `pull` / `observe` / `capsule`) with zero Node dependencies, a validation whitelist (`engine/gates.json`), and the gene/capsule/event protocol.
...
├── engine/                 # evolution engine (zero-dependency seven commands + gates.json whitelist; pull = read-only bank consumption; observe = observation input face; capsule = execution audit records; TS source, run form = tsc dist)
