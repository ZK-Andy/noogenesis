...
- **新门禁直接 TS**（B0 拍板 2026-09-08）：新增校验件写 `scripts/*.mts`（显式 ESM——本包 `type:commonjs` 下裸 .ts 为 CJS 分类装不下 import；node ≥22.18 原生 type stripping 直跑，**零 devDependency**，样板 = verify-archived-agent-notes.mts / verify-postmortem-naming.mts）；存量 Python 件只在被触碰时随族迁（B1）。
