...
- **门禁族单轨 TS（B5 起，2026-09-08）**：门禁族全件 = `scripts/*.mts`（node ≥22.18 原生直跑，零运行时依赖；`gates.mts` 为含 needs/after DAG 的 TS runner，gates.json cmd 已单源重指）。TS 间跨件消费走同族 import（如 brief → tier 的 `classify`），被 import 件的入口分发必须带守卫。
...
- **lint / export-docs（C2）**：判据单源 = 根 `.oxlintrc.json`（白名单逐条理由，oxlint 钉版）+ `verify-export-docs.mts`（导出函数/类须紧邻 JSDoc，typescript 解析）；改判据即改夹具与 [code-standards](../docs/method/code-standards.md) 档位。
