...
- **--self-test 夹具纪律**：每个 verify-* 门禁自带 `--self-test`（违约必 FAIL、合规必 PASS）；改判据不同步改夹具 = 判据漂移无人知；新增判据分支须过「删该分支」变异，仍绿即无夹具。hooks/CI 消费同一夹具。
- **新门禁必须登记 `gates.json`**：未登记 = 孤儿门禁，`gates.mts --run` 与 CI 都不执行它；登记条目名即基因 evaluate 的闸名，改名会破白名单消费方。
- **门禁族单轨 TS**：门禁族全件 = `scripts/*.mts`（node ≥22.18 原生直跑，零运行时依赖；两闸例外见下；`gates.mts` 为含 needs/after DAG 的 TS runner，gates.json cmd 已单源重指）。TS 间跨件消费走同族 import（如 brief → tier 的 `classify`），被 import 件的入口分发必须带守卫。
...
- **lint / export-docs / package-invariants（C2/B2）**：判据单源 = 根 `.oxlintrc.json` + `verify-export-docs.mts`（导出须紧邻 JSDoc）+ `verify-package-invariants.mts`（发布面不变量；缺 dist → fail-closed）；改判据即改夹具与 [code-standards](../docs/method/code-standards.md) 档位。
