...
- [ ] （A）批次表（46 项）开工中：**批次 8 连续推进**（序 40 踩坑原子化裁决落，[ADR](.agents/notes/implemented/architecture/2026-09-14-pitfall-atomization-verdict.md)：每坑一原子已交付 = cookbook、「cookbook 条 → 基因」批量 / 自动转化判不立）；游标 = **序 41**（开发流程选择域补全）。单源 = [批次表 ADR](.agents/notes/proposed/architecture/2026-09-13-feature-completion-backlog.md)。
