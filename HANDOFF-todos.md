...
- [ ] （A）设计稿功能补全批次表（46 项有序）逐批开工中：**批次 1 序 1–5、批次 2 序 6–10 与批次 3 序 11–14 已全部收口**（逐行结论与 ADR 指针 = 批次表行备注），下一件 = 批次 3 序 15 情境按需注入（命中节动态信号；仍受「自动 Detect 默认关」约束、须自身过 HERO）；批次 4 同须先立重拍 ADR。进度、覆盖来源与各批 ADR 指针单源 = [批次表 ADR](.agents/notes/proposed/architecture/2026-09-13-feature-completion-backlog.md)。
