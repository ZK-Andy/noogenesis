...
- [ ] （D）护栏建设轮（2026-09-13 立项讨论轮已产出，实现轮待开）：判据与拍板单源 = [立项 ADR](.agents/notes/proposed/architecture/2026-09-13-guardrail-construction-round.md)。三件口径 = token 基线两轨 / 改进度量两本账 / canary 判不立。实现轮余项：js-yaml 归口、Detect 信号源轮立项（D2 逐源解除，连带 M3 重议）；待证点 = 在环反馈是否让模型更合规。
