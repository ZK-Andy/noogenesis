...
- [ ] （A）M1 实现批 ×2（设计拍板 ADR [2026-09-10-m1-guard-anti-overdesign](.agents/notes/proposed/architecture/2026-09-10-m1-guard-anti-overdesign.md) proposed）：批 A = 守卫三件套（A2 技能路标 + tool-pre 触点提醒 advice 不阻断 + session-close 对账步）；批 B = 防过度混合 D（根 AGENTS 精简契约块 + docs/method 新篇 + cases 辩论索引）。判级预计 FULL，各自带评审与 ADR 翻转。
