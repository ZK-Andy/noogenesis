...
- [ ] （D）Capsule `blast_radius` 字段复议（序 4 判不立，显式挂起）：触发 = 出现按执行记录横向比较影响面的消费者（评分 / 候选比较 / 跨机贡献复验 / 合并自校验证据）且同时定义区间锚点；复议先答三问（谁生产 / 锚点用什么 / 谁消费），字段用途清单与判据单源 = [序 4 ADR](.agents/notes/implemented/architecture/2026-09-13-evaluate-blast-radius.md) B3。
...
- [ ] （A）设计稿功能补全批次表（46 项有序）逐批开工中：**批次 1 序 1–4（Capsule / Mutation / Event 扩字段 / blast-radius）已收口**（[序 4 ADR](.agents/notes/implemented/architecture/2026-09-13-evaluate-blast-radius.md)），下一件 = 序 5 候选比较（先定判据再动手）；批次 3/4 须先立重拍 ADR。单源 = [批次表 ADR](.agents/notes/proposed/architecture/2026-09-13-feature-completion-backlog.md)（proposed）。
