...
- [ ] （D）延后 ADR 终局态治理：`2026-09-06-guardrail-defer-trigger` 正文声明「不再有追随意向、保留为已消耗的提案记录」，却仍 `Status: proposed` 驻 `proposed/`——生命周期闭集（[notes/README](.agents/notes/README.md)）无此终局态。修法 = 按取代规则合并删旧 + 改入站链接（其决定与备选已由护栏建设轮 ADR 承接），或按 rejected 保留（判据 = 是否仍能防重蹈覆辙）。触发 = 下次 ADR 治理批。出处 = 护栏轮余项收尾批 R3 S9（未随批执行）。
