...
- [x] （A）编码强制两轨：批 1 落地闭环（FULL 三审全采纳；ADR `2026-09-08-lint-in-loop-feedback`；CI 34249821201 绿）；批 2 实现+门禁+负向手验全绿、**评审通道未交付故未推送**（本地 `30fa01c`）；批 3 测量 → 延后（ADR `2026-09-08-coding-enforcement-track-b`）。
- [ ] （B）批 2 评审恢复 + push：独立三审在本会话连续 8 次未交付（跨 7 轮）——新会话重跑 R1/R2/R3 → 补 track-b ADR `Review:` 行 → push `30fa01c` → CI 绿。批 1 先例：R1/R2 一轮内交付、R3 可能 4+ 轮。
- [ ] （B）批 1 真机复验：装机后新会话写含 `var` 的 `.ts` → 同轮出现 lint 反馈；干净代码 → 零反馈。前置 = 发条新版本 + 用户重装。
