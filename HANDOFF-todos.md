...
- [ ] （D）P1 骨架后置项两条（D1 遗留面 + D2 禁区；非护栏）：`js-yaml` 例外权归口（M2 未动用该权，随贡献开放轮拍）+ Detect 信号源轮立项（M2 已落地 ⇒ 禁区条件到期，逐源解除 `agent/error` / `agent/turn-stopping` / `tool/result` 等，连带 M3 重议）。触发 = 用户点名 / 真实场景出现。单源 = [P1 骨架 ADR](.agents/notes/implemented/architecture/2026-09-05-p1-engine-skeleton.md) 遗留面与 D2（内含 P2/M2 指向）。
- [ ] （D）宿主 peer 集升代批（`0.1.0-rc.8` → `0.1.5-rc.x` 全件同代 + lock 同提交）：完成后把常驻注入读数升为 `host-api-contract.mts` 的类型契约断言（当前 = 运行期形状闸）。触发判据与证据 = [护栏建设轮 ADR](.agents/notes/implemented/architecture/2026-09-13-guardrail-construction-round.md) 决定 1 勘误；现象与规避 = [cookbook](docs/cookbook.md)「环境」条。
