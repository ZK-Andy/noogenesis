...
- [ ] （A）设计稿功能补全批次表（46 项有序）逐批开工中：**批次 1 序 1–5、批次 2 序 6–10 与批次 3 序 11–15 已全部收口**（逐行结论与 ADR 指针 = 批次表行备注）；批次 3 余序 16（依赖序 18）/ 17（重议触发未到）无未过门槛的可办面，故下一件 = **批次 4「记忆线第二期」重议**（先立独立重拍 ADR；解锁 18/19/20 并连带解锁 16）。进度、覆盖来源与各批 ADR 指针单源 = [批次表 ADR](.agents/notes/proposed/architecture/2026-09-13-feature-completion-backlog.md)。
