...
- [ ] （D）`capsules/` 路径指代两义待拍板：主设计 §8.2/§9.1 指知识原子（content-addressable capsule），共享层设计稿 §6.2 指 `Capsule` 原语（`gene_ids`/`steps`/`evidence`），adapters 面「按胶囊过滤」指内容包成员集。触发 = 共享库或 `capsules/` 目录立项时随第一批拍板。单源 = ADR [2026-09-11-capsule-term-layering](.agents/notes/implemented/process/2026-09-11-capsule-term-layering.md) Decision 5。
