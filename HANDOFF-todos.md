...
- [ ] （D）Detect 信号源逐源裁决 + M3 重议：站立规则 = 自动 Detect 默认关 + 逐源门槛（重拍件 = [序 11 ADR](.agents/notes/implemented/architecture/2026-09-13-detect-source-verdict-session-event.md)，逐源结论与 ADR 指针 = 批次表行备注）；`session/event` / `agent/error` / `agent/turn-stopping` 已判不立，余 `tool/result` 待逐源过 HERO 两问；连带 M3（频次抬升 = 重开信号）。
...
- [ ] （A）设计稿功能补全批次表（46 项有序）逐批开工中：**批次 1 序 1–5、批次 2 序 6–10 与批次 3 序 11–13 已全部收口**（逐行结论与 ADR 指针 = 批次表行备注），下一件 = 批次 3 序 14 Detect 逐源裁决 `tool/result`（D2 禁区重拍件已立，站立规则 = 自动 Detect 默认关 + 逐源门槛）；批次 4 同须先立重拍 ADR。进度、覆盖来源与各批 ADR 指针单源 = [批次表 ADR](.agents/notes/proposed/architecture/2026-09-13-feature-completion-backlog.md)。
