...
- [ ] （A）术语口径：胶囊（内容包，如胶囊 01「AI 协作编码方法论」）与 Capsule 原语（设计稿 §5.1 的"一次执行的审计记录"，后置未实现）同名两义，易与 Gene 原语（`genes/*.json` 策略件）混淆——是否在 durable 文档显式区分（用户 2026-09-11 提出，尚未拍板）。单源参照：主设计 §5.1:167-168 + schema ADR:37。
