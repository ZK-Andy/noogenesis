...
- [ ] （A）设计稿功能补全批次表（46 项有序）逐批开工中：**批次 1 进行中——序 1（Capsule 原语）已收口**（[Capsule ADR](.agents/notes/implemented/architecture/2026-09-13-capsule-primitive.md)），下一件 = 序 2 Mutation 原语；批次 3/4 须先立重拍 ADR（Detect 禁区、记忆线判不立）。单源 = [批次表 ADR](.agents/notes/proposed/architecture/2026-09-13-feature-completion-backlog.md)（proposed）。
