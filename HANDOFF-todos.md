...
- [ ] （D）Capsule `blast_radius` 字段复议（序 4 判不立，显式挂起）：触发 = 出现按执行记录横向比较影响面的消费者（跨机贡献复验 / 合并自校验证据）且同时定义区间锚点；复议先答三问（谁生产 / 锚点 / 谁消费），判据单源 = [序 4 ADR](.agents/notes/implemented/architecture/2026-09-13-evaluate-blast-radius.md) B3——评分类与候选比较类已判不再构成触发（[序 5 ADR](.agents/notes/implemented/architecture/2026-09-13-candidate-comparison-blindness.md)）。
...
- [ ] （D）Detect 信号源逐源解除 + M3 重议：M2 已拍板「Detect 显式不做」，禁区仍在；解除须显式重拍——逐源裁决 `session/event` / `agent/error` / `agent/turn-stopping` / `tool/result`，并先答「检测到什么具体失败 → 下一步做什么不同」；连带 M3（调用频次抬升 = 重开信号）。触发 = 用户点名 / 真实场景出现。单源 = [P1 骨架 ADR](.agents/notes/implemented/architecture/2026-09-05-p1-engine-skeleton.md) D2 遗留面。
...
- [ ] （A）设计稿功能补全批次表（46 项有序）逐批开工中：**批次 1 序 1–5 已收口**（[序 5 判据 ADR](.agents/notes/implemented/architecture/2026-09-13-candidate-comparison-blindness.md)），下一件 = 批次 2 序 6 A5 其余三时刻；批次 3/4 须先立重拍 ADR。单源 = [批次表 ADR](.agents/notes/proposed/architecture/2026-09-13-feature-completion-backlog.md)（proposed）。
