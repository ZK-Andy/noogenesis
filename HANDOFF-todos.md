...
- [ ] （C）npm 0.1.2 可选首发：适配层 `geneBankUrl` 惰性 pull + 技能随库分发（skills-ride-bank）尚未进 desktop 部署（在装 0.1.1）；发版走 release-flow（minimumReleaseAge 豁免命令见 cookbook [环境]），发后 desktop 重装 + 新会话重验两件事：geneBankUrl 面（pull 命中/降级）+ 技能面（rank 600 遮蔽关系、缓存缺席降级、invalidate 刷新）；不发也不影响现有功能（P2 消费在引擎/CLI 面已完整）。
