...
- [ ] （C）异步演化轮（演化动作面的归集入口）：攒账 = 每批评审的可机械判类 / 纪律漏项 / 带症状根因的踩坑，由主会话当场写一行进本区；触发 = 攒到可拍板或用户点名；处理 = 成批拍板落闸 / 落卡 / 落条目（门槛单源 = [发现机械化 ADR](.agents/notes/implemented/process/2026-09-11-review-finding-mechanization.md) Decision 1；决策与缺口 = [撤除 ADR](.agents/notes/implemented/process/2026-09-12-absorption-async-round.md)）。
- [ ] （C）bank 技能 provider 注册失效（2026-09-12 实机取证：desktop 199/199 会话目录 `noo-*`=0、自举仓 262/262 由文件系统 provider 掩盖；provider 代码与缓存均正常）：三件同批 = `inject` 补 `skills` + 可重试注册 / 提醒面目录感知（A2 行 + `skillGuards` 默认值）/ 外仓 marker 实证夹具。立项件 = [proposed](.agents/notes/proposed/bug-fix/2026-09-12-bank-skill-provider-registration.md)；触发 = 演化轮开轮或用户点名。
