...
- [ ] （D）M2 适配层立项讨论轮（新项目开发前）：前置事实（2026-09-05 实测）——本仓尚非 DSH 插件包（缺 package.json / cordis.patch.yml / plugin row / lib 入口，对照 dsh-continual-evolve@0.6.0 安装形态 `dsh plugin add`）；noo-* 技能已在 DSH 侧自动发现（已通），断的是引擎↔会话生命周期接线；正式发布 gate 拟 = `dsh plugin add noogenesis` 可装且装上即转。讨论待拍板题：插件形态/接线点/进程内 vs spawn CLI/技能分发载体。
