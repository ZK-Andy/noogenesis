...
- [ ] （C）0.1.3 已发布待用户 desktop 重装重启（2026-09-06，npm latest + tag `v0.1.3`）：余项 = 新会话重验两件事——geneBankUrl 面（默认零配置拉官方库进会话仓 genes-cache + remote 核验；降级面已实证）+ 技能面（rank 600 遮蔽、invalidate 刷新；缺席降级已实证）；通过即清账开胶囊 01 优化轮。前置 = 装载期 pull 错位缺陷已修（ADR 2026-09-06-bank-pull-session-trigger implemented）。
