...
- [ ] （B）技能触点提醒触发面扩面真机复验（触发 = 含本批版本装机后新会话）：写 `.md` 未载 prose-standard 得一行 advice；`git push` 未载 pre-push-checks 得 advice（含 `cd X && git push` / `timeout N git push`）；bash 重定向写 `journal/*.md` 命中；已载不重提；异仓如实记。ADR [触发面扩面](.agents/notes/implemented/architecture/2026-09-11-skill-guard-trigger-faces.md)；push 面正则条见 [池件](HANDOFF-evolution-pool.md)。
