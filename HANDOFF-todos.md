...
- [ ] （A）批次表（46 项）开工中：**批次 6 开门重拍已落**（P1 D3 重议实质维持；序 30 蒸馏以「显式触发 + 压缩在宿主侧 + 产物是候选」放行——[重拍 ADR](.agents/notes/implemented/architecture/2026-09-14-p1-d3-distillation-reshoot.md)）；游标 = **序 30 实现轮**（feature-flow 独立立项）。单源 = [批次表 ADR](.agents/notes/proposed/architecture/2026-09-13-feature-completion-backlog.md)。
