...
- [x] （C）2026-09-11 两件：`session-close` §10「吸收账回执」字段随阶段卡结构批兑现、后随 [撤除 ADR](.agents/notes/implemented/process/2026-09-12-absorption-async-round.md) 撤除；`verify-review-brief` 增「简报 head 须钉住 HEAD」判据（夹具 13→14）。
...
- [x] （B）护栏读数建议行真机复验 2026-09-16（0.2.8 装机）：落盘文件两会话各恰一行读数（15624 / 316694）、零 warn；「随会话增长」半条撤销（每会话至多一行）；服务缺席 / `measure` 抛错两面真机不可达，self-test 124 组钉住。[复验落账](.agents/notes/implemented/bug-fix/2026-09-16-plugin-log-sink.md)。
- [x] （B）A6 停止前能力位真机复验 2026-09-16（同上装机）：`turn/end` 正常关回合；落盘文件零 `turn-stopping mount failed` / `turn-stop steering` warn、listener 零诊断。[复验落账](.agents/notes/implemented/architecture/2026-09-13-a6-turn-stopping-mount.md)。
