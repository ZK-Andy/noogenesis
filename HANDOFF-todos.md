...
- [ ] （A）M1 批 B 防过度混合 D（Decision 2 已随批 A 翻转，ADR [2026-09-10-m1-guard-anti-overdesign](.agents/notes/implemented/architecture/2026-09-10-m1-guard-anti-overdesign.md)）：根 AGENTS 精简契约块（HERO ~8 条 + 核心判据 + Rule 8，≤120 词）+ docs/method 新篇（9 规则 + 6 校准形状 + 同域归并，manifest 预算）+ cases 索引落 docs/research/（不进常驻/懒加载面）。判级 FULL；收口补第二条 Review 行。
