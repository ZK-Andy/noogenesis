...
- [x] （D）M2 适配层立项讨论轮 2026-09-06 四题拍板（插件壳/最小接线/spawn 单合同/技能分发随 P2），ADR [2026-09-06-m2-adapter-wiring](.agents/notes/proposed/architecture/2026-09-06-m2-adapter-wiring.md)。
- [ ] （D）M2 适配层实现轮：按 ADR [2026-09-06-m2-adapter-wiring](.agents/notes/proposed/architecture/2026-09-06-m2-adapter-wiring.md) 实施——插件壳（package.json + cordis.patch.yml + ESM lib）+ 接线四件（system-prompt 节 / noo 三工具 / solidify 默认人工确认 / /noo 后补）+ spawn 单合同（退出码 1 闸红 / 2 fail-closed）；附带 hooks/CI 改引 gates.json；gate = 可装且空 genes/ 优雅退化。
