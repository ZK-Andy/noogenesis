...
- [ ] （A）批次表（46 项）开工中：**批次 8 连续推进**（序 41 开发流程选择域裁决落，[ADR](.agents/notes/implemented/architecture/2026-09-14-dev-process-selection-verdict.md)：域内容单源既有家、四项候选判不立）；游标 = **序 42**（知识原子 content-addressable）。单源 = [批次表 ADR](.agents/notes/proposed/architecture/2026-09-13-feature-completion-backlog.md)。
