...
- [ ] （B）护栏读数建议行真机复验（触发 = 下次发版装机后；观察面 = [落盘通道](.agents/notes/implemented/bug-fix/2026-09-16-plugin-log-sink.md)）：`<DSH_HOME>/logs/noogenesis.log` 每会话自第二步起恰一行 `noogenesis token baseline reading: session=<id> surfaceTokens=<n>`（非零、随会话增长；session= 字段 sort | uniq -c 复算「每会话恰一行」），无 token-meter 的 profile 零 warn、measure 抛错时恰一条 warn。
