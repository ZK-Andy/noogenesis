...
- [ ] （A）设计稿功能补全批次表（46 项有序）逐批开工中：**批次 1 序 1–5 与批次 2 序 6–10 已全部收口**（逐行结论与 ADR 指针 = 批次表行备注），下一件 = 批次 3 序 11 Detect 逐源裁决 `session/event`（**须先立 P1 D2 禁区 + M2「Detect 显式不做」的重拍 ADR**）；批次 4 同须先立重拍 ADR。进度、覆盖来源与各批 ADR 指针单源 = [批次表 ADR](.agents/notes/proposed/architecture/2026-09-13-feature-completion-backlog.md)。
