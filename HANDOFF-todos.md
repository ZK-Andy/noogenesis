...
> 记忆库线（含痕迹提炼面 D 条）已迁出（2026-09-10）：唯一状态家 = [docs/state/memory-system-dossier.md](docs/state/memory-system-dossier.md)（档案页制度，2026-09-13 收口）。
...
- [ ] （B）护栏读数建议行真机复验（触发 = 下次发版装机后；观察面 = [落盘通道](.agents/notes/implemented/bug-fix/2026-09-16-plugin-log-sink.md)）：`<DSH_HOME>/logs/noogenesis.log` 每会话自第二步起恰一行 `noogenesis token baseline reading: session=<id> surfaceTokens=<n>`（非零、随会话增长；session= 字段 sort | uniq -c 复算「每会话恰一行」，warn 行同带该字段），无 token-meter 的 profile 零 warn、measure 抛错时恰一条 warn。
- [ ] （B）A6 停止前能力位真机复验（触发 = 下次发版装机后）：装机会话正常关回合、`<DSH_HOME>/logs/noogenesis.log` 零 `turn-stopping mount failed` / `turn-stop steering` warn（零策略面应为零投递零 warn；该判据的观察面同 0.2.7 修正 = 插件日志落盘 ADR）；listener 注册无诊断。判据与降级口径单源 = [A6 能力位 ADR](.agents/notes/implemented/architecture/2026-09-13-a6-turn-stopping-mount.md) Consequences。
- [ ] （D）Capsule `blast_radius` 字段复议（序 4 判不立，显式挂起）：触发 = 出现按执行记录横向比较影响面的消费者（跨机贡献复验；「合并自校验证据」半边已随序 45 裁决关闭）且同时定义区间锚点；复议先答三问（谁生产 / 锚点 / 谁消费），判据单源 = [序 4 ADR](.agents/notes/implemented/architecture/2026-09-13-evaluate-blast-radius.md) B3（评分与候选比较已判不再构成触发，见该处）。
...
- [ ] （D）`js-yaml` 例外权与基因格式 YAML 化归口：该权 M2 未动用，产出取决于是否真做 YAML 化/schema 扩字段（后置清单 = [P2 ADR](.agents/notes/implemented/architecture/2026-09-06-p2-shared-consumer.md) D3）。触发 = 贡献开放轮开轮时随第一批拍。单源 = [P1 骨架 ADR](.agents/notes/implemented/architecture/2026-09-05-p1-engine-skeleton.md) 遗留面。
- [ ] （D）Detect 信号源逐源裁决 + M3 重议：站立规则 = 自动 Detect 默认关 + 逐源门槛（重拍件 = [序 11 ADR](.agents/notes/implemented/architecture/2026-09-13-detect-source-verdict-session-event.md)，逐源结论与 ADR 指针 = 批次表行备注）；四源（行 11–14）**已全判不立**、裁决组收口；连带 M3（频次抬升 = 重开信号）。
- [x] （D）宿主 peer 集升代批 2026-09-13（FULL 三审 R1 0B/5S、R2 0B/2S、R3 1B/3S；采纳 10 拒绝 1）：全件同代 `0.1.5-rc.2` + tokenMeter 类型契约断言（运行期形状闸保留）；[ADR](.agents/notes/implemented/architecture/2026-09-13-host-peer-generation-upgrade.md)。
- [x] （D）ADR 终局态治理 2026-09-13 收口：`guardrail-defer-trigger` 删除（决定与备选由护栏建设轮 ADR 承接）+ 入站链重定向；`collab-rebuild-impl` 转 implemented；[治理 ADR](.agents/notes/implemented/process/2026-09-13-adr-terminal-state-governance.md)。
- [x] （D）`capsules/` 路径指代两义 2026-09-13 随批次 1 序 1 收口：裁定 `capsules/` 只指 Capsule 原语（执行审计记录），§8.2 知识原子不落此处、归序 42；序 27 随之收窄为确认此口径。[Capsule ADR](.agents/notes/implemented/architecture/2026-09-13-capsule-primitive.md) C1。
...
- [x] （C）档案页制度批 2026-09-13 收口（FULL 三审 R1 2B/2S、R2 1B/1S、R3 3B/2S）：立 `docs/state/` tier + 骨架四段机器闸 `verify-dossier-format` + 摩擦点落点表；推广 = 判据立、页不立。[ADR](.agents/notes/implemented/process/2026-09-13-dossier-institution.md)。
