...
- [ ] （D）协作层重建实现轮（立项 ADR [2026-09-06-collab-rebuild-impl](.agents/notes/proposed/architecture/2026-09-06-collab-rebuild-impl.md)：B0 框架结构面批待拍板（推荐插入）+ B1→B5 按序、lefthook、0.2.0 断点）：下会话首题 = B0 拍板，随后逐批实现；每批 FULL 三审 + 蓝图 C1–C15 对账 + 事件轨标注；B5 删旧机器件并发 0.2.0。
