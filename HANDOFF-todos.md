...
- [ ] （D）P2 实现轮（立项已拍板，ADR [2026-09-06-p2-shared-consumer](.agents/notes/proposed/architecture/2026-09-06-p2-shared-consumer.md) proposed）：共享客户端 pull 消费（`engine/` 新命令，零依赖，git 子进程，缓存并入 select 扫描根）+ manifest 最小索引（生成器 + 门禁）+ 跨仓 genes/ 合并扫描；默认离线，显式配置才接库（夹具验证）；实现轮拍板缓存 vs repoRoot 链优先级与同名冲突语义。后置：capsules/、schema/YAML、观测透镜、gene→skill、Genesis 基因。
