...
- [ ] （A）批次表（46 项）开工中：**批次 8 连续推进**（序 42 知识原子地址面裁决落，[ADR](.agents/notes/implemented/architecture/2026-09-14-knowledge-atom-address-verdict.md)：内容三既有家、机器地址面 Event SHA、逐原子地址判不立）；游标 = **序 43**（子代理自动执行验证）。单源 = [批次表 ADR](.agents/notes/proposed/architecture/2026-09-13-feature-completion-backlog.md)。
