...
- [ ] （D）护栏建设轮（ADR 已转 implemented；余项待做）：判据与拍板单源 = [护栏建设轮 ADR](.agents/notes/implemented/architecture/2026-09-13-guardrail-construction-round.md)。已落 = 常驻注入字面预算判据（装载期 fail-closed）；**余项** = 宿主读数建议行（`ctx.tokenMeter` 懒取用 + 形状断言 + 缺席降级，须扩适配层允许集并拍板）、两本账落点（三处归口同步已随收口批兑现）；另 js-yaml 归口、Detect 信号源轮立项（D2 逐源解除，连带 M3 重议）；待证点 = 在环反馈是否让模型更合规。
