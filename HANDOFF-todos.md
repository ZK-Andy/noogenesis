...
- [ ] （B）bank provider 注册修复真机复验（2026-09-13 修复批，装机后跑）：判据 = 含 `.noogenesis/genes-cache` 技能的外仓里，新会话 `<available_skills>` 出现 `noo-*`（来源 = `noogenesis-bank` provider），且 A2 路标行与触点提醒只点名实际可载技能；触发 = 本批发版 + 用户重装重启。修复件 = [ADR](.agents/notes/implemented/bug-fix/2026-09-12-bank-skill-provider-registration.md)。
