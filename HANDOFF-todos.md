...
- [ ] （B）技能触点提醒触发面真机复验（触发 = 0.2.5 装机后新会话）：`.md`→prose-standard、`git push`→pre-push-checks 各得一行 advice（push 面含 `cd X &&` / `timeout N` / `-c k=v` 三类前导形态）；bash 重定向写 `journal/*.md` 命中；已载不重提；异仓如实记。ADR [触发面扩面](.agents/notes/implemented/architecture/2026-09-11-skill-guard-trigger-faces.md)。
...
- [ ] （C）release-note 大批次归并档（**触发已满足 = 2026-09-12 0.2.5 发版二次实证**：跨 67 笔时「其他变更」节输出 42 条，发布者手工归并 14 条）：修法 = 脚本增按批次聚合档；发版会话不得顺手改发布脚本（[release-flow](.agents/workflows/release-flow.md) 硬规则），走 feature-flow 独立批。证据 = [release-shape-alignment](.agents/notes/implemented/process/2026-09-09-release-shape-alignment.md) 第二次实发节。
