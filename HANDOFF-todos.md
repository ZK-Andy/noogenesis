...
- [ ] （B）技能触点提醒触发面真机复验（触发 = 装机后新会话）：`.md`→prose-standard、`git push`→pre-push-checks 各得一行 advice（push 面含 `cd X && git push` / `timeout N git push` / `git -c credential.helper=… push` 三形态）；bash 重定向写 `journal/*.md` 命中；已载不重提；异仓如实记。ADR [触发面扩面](.agents/notes/implemented/architecture/2026-09-11-skill-guard-trigger-faces.md)。
