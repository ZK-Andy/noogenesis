...
- [ ] （A）批次表（46 项）开工中：**批次 1–4 已收口**（重拍：序 18 判已交付、序 19 维持判不立、序 20/16/17 维持——[重拍 ADR](.agents/notes/implemented/architecture/2026-09-14-memory-line-phase2-reshoot.md)）；游标 = **批次 5（序 21–29）**：序 21/22 前提未满足；可办面 = 拍板件序 23/24/28/29 + 确认件序 27。单源 = [批次表 ADR](.agents/notes/proposed/architecture/2026-09-13-feature-completion-backlog.md)。
